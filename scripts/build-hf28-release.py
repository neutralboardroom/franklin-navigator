...
RELEASE = 'FR-NAV1.17.0-HF2.8-CANDIDATE'
FILENAME = 'FRANKLIN_NAVIGATOR__HF2_8__FIND_LOCAL_SIMPLIFIED__2026-09-09.zip'
...
def build_bytes(source_commit: str) -> tuple[bytes, dict, bytes, bytes]:
    ...
    exclusions = json.dumps({
        'schemaVersion': 'franklin.hf28.release-exclusions.v1',
        'release': RELEASE,
    ...
    contents['HF28_RELEASE_EXCLUSIONS.json'] = exclusions
    rows.append({'path': 'HF28_RELEASE_EXCLUSIONS.json', 'bytes': len(exclusions), 'sha256': sha_bytes(exclusions)})
    rows.sort(key=lambda x: x['path'])

    manifest = {
        'schemaVersion': 'franklin.hf28.release-manifest.v1',
        'release': RELEASE,
        'sourceCommit': source_commit,
    ...
    contents['HF28_RELEASE_MANIFEST.json'] = manifest_bytes
...
def validate_archive(path: pathlib.Path, manifest: dict):
    with zipfile.ZipFile(path) as zf:
        ...
        expected = {r['path'] for r in manifest['files']} | {'HF28_RELEASE_MANIFEST.json'}
        if set(names) != expected:
            raise SystemExit('ZIP member set does not match manifest')

    with tempfile.TemporaryDirectory(prefix='hf28-extract-') as td:
        dest = pathlib.Path(td)
        with zipfile.ZipFile(path) as zf:
            zf.extractall(dest)
        subprocess.run(['python3', 'scripts/validate-hf28-release.py', '.'], cwd=dest, check=True)
        subprocess.run(['node', '--check', 'dist/assets/hf27-navigation.js'], cwd=dest, check=True)
        subprocess.run(['node', '--check', 'dist/assets/hf28-directory.js'], cwd=dest, check=True)
...
def main():
    ...
    if release_meta.get('releaseState') not in {'LIVE_VERIFIED_QUALIFIED_PENDING_SEAL','QUALIFIED_SUCCESSOR_SEALED'}:
        raise SystemExit('HF2.8 is not live-verified/qualified for sealing')
    ...
    attestation = {
        'schemaVersion': 'franklin.hf28.external-release-attestation.v1',
        'release': RELEASE,
        'sourceCommit': source_commit,
        'artifact': FILENAME,
        ...
        'freshExtractionValidator': 'PASS',
        'freshExtractionNodeSyntax': 'PASS',
        'freshExtractionFindLocalSourceQualification': 'PASS',
        'nestedArchives': 0,
        'authorityTransfer': False
    }
    attestation_path = outdir / 'HF28_EXTERNAL_RELEASE_ATTESTATION.json'
