...
async function inspect(client,cfg){
  ...
  try{
    ...
    if(ok){
      ...
      const recentAuth=await client.query(`SELECT count(*)::integer AS recent_auth_events,max(created_at) AS latest_auth_at FROM franklin_audit_log WHERE actor_type='ACCOUNT' AND actor_ref_hash=$2 AND action_type='REVIEW_SESSION_CREATED' AND created_at>now()-interval '6 hours'`,[cfg.accountId,digest(cfg.accountId)]);
      ...
      const rs=reviewSession.rows[0],ra=recentAuth.rows[0],as=regularSession.rows[0],lc=lifecycle.rows[0];
