...
for p in DIST.rglob('*.html'):
    ...
    if s.select_one('[data-community-explorer]'):
        ...
        finder=s.select_one('#finder');hero=s.select_one('.explorer-hero')
        if finder and hero and finder.previous_sibling is None:issues.append((r,'finder_order'))
...
if not (DIST/'assets/hf34.css').exists():issues.append(('/assets','css_missing'))
if not (DIST/'assets/hf34-explorer.js').exists():issues.append(('/assets','explorer_js_missing'))
if not (DIST/'assets/hf34-member-public.js').exists():issues.append(('/assets','member_js_missing'))
