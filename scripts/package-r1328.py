...
def tracked_files():
    ...
    for b in raw.split(b'\0'):
        ...
        if p==MANIFEST: raise SystemExit(f'{MANIFEST} must be generated, not tracked')
...
def main():
    ...
    commit=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(); files=tracked_files()
    with tempfile.TemporaryDirectory(prefix='r1328-stage-') as td:
        staging=pathlib.Path(td); manifest=stage(staging,files,commit)
        ...
        att={
          ...
          'deterministicDoubleBuild':'PASS_BYTE_IDENTICAL','manifest':MANIFEST,
          'manifestMembersExcludingSelf':manifest['memberCountExcludingManifest']
        }
