...
MANIFEST = ROOT / 'HF28_RELEASE_MANIFEST.json'
...
if not MANIFEST.is_file():
    raise SystemExit('HF28 manifest missing')
manifest = json.loads(MANIFEST.read_text('utf-8'))
if manifest.get('release') != 'FR-NAV1.17.0-HF2.8-CANDIDATE':
    raise SystemExit('HF28 release identity mismatch')
if manifest.get('manifestSelfHashRule') != 'MANIFEST_FILE_EXCLUDED_FROM_ITS_OWN_MEMBER_HASH_TABLE':
    raise SystemExit('HF28 self-hash rule missing')
...
for path in ROOT.rglob('*'):
    ...
    if rel == 'HF28_RELEASE_MANIFEST.json':
        continue
    actual[rel] = path
