...
def replace(rel,old,new):
    ...
    if old in s: p.write_text(s.replace(old,new),'utf-8')

# Retired lookup keys must not remain in downloadable public JS. Historical accounts are
# labeled from their opaque server-returned key without publishing retired identifiers.
...
('<h2>Free correction remains separate</h2><p>Anyone may suggest a factual correction without paying or becoming a member.</p>','<h2>Free profile controls remain separate</h2><p>Basic factual corrections are free. You can also request removal of your profile from public view at no cost. No membership or payment is required.</p><div class="actions"><a class="button" href="/corrections/">Correct profile information</a><a class="button" href="/corrections/?action=PUBLIC_REMOVAL">Request removal from public view</a></div>'),
...
for rel,pairs in changes.items():
    for old,new in pairs: replace(rel,old,new)
...
if "'One simple membership.':'Una membresía sencilla.'" not in s and needle in s:
    fixed="""    const fixed={
