...
const chrome=spawn(chromePath,['--headless=new','--no-sandbox','--disable-gpu','--disable-dev-shm-usage','--remote-debugging-address=127.0.0.1','--remote-debugging-port=0','--user-data-dir='+userDir,'about:blank'],{stdio:['ignore','ignore','pipe']});
...
let target;
for(let i=0;i<100;i++){try{target=(await json('http://127.0.0.1:'+cdpPort+'/json/list')).find(x=>x.type==='page');if(target)break}catch{}await sleep(100)}
...
await waitFor("[...document.querySelectorAll('*')].some(n=>n.textContent?.trim()==='Official Franklin Navigator profile')","official Franklin Navigator indicator");
const officialPub=await evaluate(`(()=>{const links=[...document.querySelectorAll('a')],claim=links.find(a=>a.textContent.trim()==='Manage this profile — free'),correct=links.find(a=>a.textContent.trim()==='Correct factual listing details'),official=[...document.querySelectorAll('*')].find(n=>n.textContent?.trim()==='Official Franklin Navigator profile');const vis=n=>{if(!n)return false;const r=n.getBoundingClientRect(),st=getComputedStyle(n);return r.width>1&&r.height>1&&st.display!=='none'&&st.visibility!=='hidden'};return{claim:vis(claim),correct:vis(correct),official:vis(official),correctHref:correct?.getAttribute('href')}})()`);
...
await waitFor("[...document.querySelectorAll('a')].some(a=>a.textContent.trim()==='Manage this profile — free')","ordinary public management CTA");
await waitFor("[...document.querySelectorAll('a')].some(a=>a.textContent.trim()==='Correct factual listing details')","ordinary public correction route");
const pub=await evaluate(`(()=>{const links=[...document.querySelectorAll('a')],claim=links.find(a=>a.textContent.trim()==='Manage this profile — free'),correct=links.find(a=>a.textContent.trim()==='Correct factual listing details');const vis=n=>{if(!n)return false;const r=n.getBoundingClientRect(),st=getComputedStyle(n);return r.width>1&&r.height>1&&st.display!=='none'&&st.visibility!=='hidden'};return{claim:vis(claim),claimHref:claim?.getAttribute('href'),claimClass:claim?.className,correct:vis(correct),correctHref:correct?.getAttribute('href'),claimRect:claim?claim.getBoundingClientRect().toJSON():null}})()`);
