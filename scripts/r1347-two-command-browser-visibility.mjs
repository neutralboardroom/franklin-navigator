...
const officialPub=await evaluate(`(()=>{const state=document.querySelector('.r1330-state-label'),action=document.querySelector('a.r1330-claim-link'),correct=document.querySelector('#manage [data-r1338-correct]');const vis=n=>{if(!n)return false;const r=n.getBoundingClientRect(),st=getComputedStyle(n);return r.width>1&&r.height>1&&st.display!=='none'&&st.visibility!=='hidden'};return{claim:vis(action)&&action.textContent.trim()==='Manage this profile — free',correct:vis(correct),official:vis(state)&&state.textContent.trim()==='Official Franklin Navigator profile',stateActionText:action?.textContent.trim(),correctHref:correct?.getAttribute('href')}})()`);
