(()=>{
  ...
  form.addEventListener('reset',()=>setTimeout(()=>{workspace=null;output.innerHTML='<p class="empty-state">Your 7-, 30- and 90-day plan will appear here.</p>';copy.disabled=download.disabled=print.disabled=true;status.textContent='Cleared. Nothing was saved or sent.'},0));
  const publicPlan=()=>workspace?{title:`${workspace.verticalLabel} 90-day business growth plan`,businessType:workspace.verticalLabel,goal:workspace.goalLabel,timeAvailable:workspace.capacityLabel,listingChecklistScore:`${workspace.profileAudit.score}/100`,actions:workspace.nextBestActions.map(item=>({when:item.cadence,action:item.title,why:item.why,beforeYouAct:item.state==='Saved on this device'?'Ready for you to review':item.state,tool:item.route})),progress:workspace.memberValueLedger.map(item=>item.signal),notice:'Prepared on this device. No account or paid plan was created. Nothing was saved, submitted, sent, published, purchased or charged.'}:null;
