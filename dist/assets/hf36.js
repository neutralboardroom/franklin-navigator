/* FR-NAV1.30.1-HF3.11.1 backend-first direct-answer Franklin Assistant + fail-safe logo/loader hardening */
(()=>{'use strict';
...
const loadAssistantR1301=()=>{
  if(!document.querySelector('[data-navigator-bot]')||document.querySelector('script[data-franklin-assistant-r1301]'))return;
  ...
  load('/assets/franklin-assistant-core.js?v=frnav1292','franklin-assistant-core')
    ...
    .then(()=>load('/assets/franklin-assistant-r1301.js?v=frnav1301','franklin-assistant-r1301'))
    .then(()=>{if(!window.FranklinAssistantR1301)throw new Error('Canonical Assistant did not initialize')})
...
const installAssistantFailureFallback=(err)=>{
  ...
  const show=(question='')=>{
    ...
    if(!d){d=document.createElement('dialog');d.className='franklin-chat-panel';d.dataset.franklinAssistantFallbackDialog='1';d.innerHTML='<div class="franklin-chat-shell"><header class="franklin-chat-head"><div><div class="eyebrow">Franklin Assistant</div><h2>Franklin Assistant needs a quick retry</h2></div><button class="franklin-chat-close" type="button" aria-label="Close Franklin Assistant">×</button></header><div class="franklin-chat-transcript" data-fallback-body></div><div class="franklin-chat-composer"><div class="franklin-chat-compose-actions"><a class="button primary" href="/#ask-navigator">Try Franklin Assistant again</a><a class="button" href="/directory/">Find Local</a><a class="button" href="/community-help-center/">Help Center</a></div></div></div>';document.body.append(d);d.querySelector('.franklin-chat-close').addEventListener('click',()=>d.close())}
