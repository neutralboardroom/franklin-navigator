...
function install(root){
  ...
  const finishTurn=(question,data)=>{if(statusTurn){statusTurn.remove();statusTurn=null}const turn=make('section','franklin-assistant-turn');turn.append(answerCard(data));output.append(turn);history.push({role:'user',content:question},{role:'assistant',content:text(data.answer)});if(history.length>12)history.splice(0,history.length-12);if(String(data.mode||'')==='directory_handoff')lastDirectoryQuery=data.directoryQuery||directoryQueryFromData(data)||lastDirectoryQuery;const shown=Array.isArray(data.profiles)?data.profiles.filter(Boolean):[];if(shown.length)lastProfiles=shown.map(p=>({id:p.id||'',name:p.name||'',category:p.category||'',location:p.location||'',profile:p.profile||'',phone:p.phone||'',phoneHref:p.phoneHref||'',websiteHref:p.websiteHref||''}));else if(String(data.mode||'')!=='error')lastProfiles=[];input.value='';input.placeholder=language==='es'?'Haga una pregunta de seguimiento…':'Ask a follow-up…';input.focus();turn.scrollIntoView({block:'nearest',behavior:'smooth'})};
