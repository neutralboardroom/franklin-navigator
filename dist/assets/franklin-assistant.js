...
const VERSION='FRANKLIN-ASSISTANT2-0.3.6';
...
const PROFILE_ID=/^FR-[A-Z0-9]+-[A-Za-z0-9][A-Za-z0-9._-]{2,100}$/;
const PROFILE_STAGES=new Set(['public_profile','claim_profile','profile_access']);
function publicProfileContext(){const p=new URLSearchParams(location.search),profile=text(p.get('profile')||''),profileName=text(p.get('profileName')||'').slice(0,160),stage=text(p.get('stage')||'');if(!PROFILE_ID.test(profile)||!profileName||!PROFILE_STAGES.has(stage))return null;return Object.freeze({profile,profileName,stage})}
function clearPublicProfileContextFromUrl(){try{const u=new URL(location.href);for(const key of ['profile','profileName','stage'])u.searchParams.delete(key);history.replaceState(history.state,'',u.pathname+(u.search?'?'+u.searchParams.toString():'')+u.hash)}catch{}}
function profileWorkflowAnswer(question,context,language){
  if(!context)return null;
  const q=text(question).toLowerCase(),es=language==='es',id=encodeURIComponent(context.profile),name=context.profileName,page=location.origin+'/profiles/'+context.profile+'/';
  const access='/profile-access/?profile='+id;
  const correction='/corrections/?listing='+encodeURIComponent(name)+'&profile='+id+'&url='+encodeURIComponent(page);
  const links=rows=>rows.map(([label,url])=>({label,url}));
  if(/after|next|pending|review|happen|despu[eé]s|siguiente|pendiente|revisi[oó]n|qu[eé] pasa/.test(q))return {answer:es?'Después de solicitar acceso, Franklin muestra el estado de la solicitud y la siguiente acción. Una solicitud puede permanecer pendiente o requerir revisión adicional antes de verificarse. No se promete un tiempo de aprobación y no se requiere una membresía pagada.':'After you request access, Franklin shows the request status and next action. A request may remain pending or require additional review before it is verified. Franklin does not promise an approval time, and paid membership is not required.',mode:'public_profile_workflow',sources:[],links:links([[es?'Revisar Acceso al Perfil':'Check Profile Access',access]])};
  if(/correct|remove|wrong|free|correg|retir|eliminar|gratis/.test(q))return {answer:es?'Sí. Las correcciones factuales y las solicitudes de retiro del perfil público son gratuitas. La Membresía Comunitaria es opcional y no cambia la verdad factual ni la autoridad sobre el perfil.':'Yes. Factual corrections and public-profile removal requests are free. Community Membership is optional and does not change factual truth or profile authority.',mode:'public_profile_workflow',sources:[],links:links([[es?'Corregir información':'Correct information',correction],[es?'Ver perfil público':'View public profile','/profiles/'+id+'/']])};
  if(/what can|what do|options|do with|puedo|opciones|qu[eé] puedo|hacer con/.test(q))return {answer:es?'Con este perfil puede ver la información pública, solicitar o recuperar acceso de administración gratis, pedir correcciones factuales o retiro gratis y, después de la autoridad aplicable, considerar la Membresía Comunitaria opcional si le resulta útil.':'With this profile you can view the public listing, request or regain management access for free, request factual corrections or removal for free, and—after applicable authority is established—consider optional Community Membership if it is useful to you.',mode:'public_profile_workflow',sources:[],links:links([[es?'Ver perfil público':'View public profile','/profiles/'+id+'/'],[es?'Solicitar acceso':'Request access',access],[es?'Corregir información':'Correct information',correction]])};
  if(/manage|claim|access|administrar|gestionar|reclamar|acceso/.test(q))return {answer:es?'Puede solicitar o recuperar acceso de administración gratis. Franklin mantiene este perfil exacto durante el inicio de sesión, la recuperación de contraseña y la revisión de autoridad. La Membresía Comunitaria no es necesaria para solicitar acceso.':'You can request or regain management access for free. Franklin keeps this exact profile selected through sign-in, password recovery, and authority review. Community Membership is not required to request access.',mode:'public_profile_workflow',sources:[],links:links([[es?'Abrir Acceso al Perfil':'Open Profile Access',access],[es?'Ver perfil público':'View public profile','/profiles/'+id+'/']])};
  return null;
}
...
function styles(){
  ...
  .franklin-assistant-chat-header{margin:0 0 10px;padding-bottom:10px;border-bottom:1px solid #d7e2e2}.franklin-assistant-chat-header .eyebrow{margin-bottom:2px}.franklin-assistant-chat-header strong{display:block;font-size:1.05rem}
  .franklin-assistant-profile-context{margin:0 0 14px;padding:12px 14px;border:1px solid #b8d1d1;border-radius:12px;background:#f4fafa}.franklin-assistant-profile-context-head{display:flex;flex-wrap:wrap;gap:8px 12px;align-items:center;justify-content:space-between}.franklin-assistant-profile-context strong{display:block}.franklin-assistant-profile-context .fine-print{margin:5px 0 8px}.franklin-assistant-profile-context .actions{margin-top:8px}
  .navigator-form.franklin-assistant-followup-composer{margin-top:14px;padding-top:14px;border-top:1px solid #d7e2e2}.navigator-form.franklin-assistant-followup-composer>label{display:block;margin-bottom:8px}.navigator-form.franklin-assistant-followup-composer textarea{margin-bottom:0}
...
function continuedDirectoryRequest(query,language){
  const q=text(query);
  if(!q)return null;
  return {
    answer:language==='es'
      ?'Buscaré más perfiles locales que coincidan con “'+q+'”.'
      :'I’ll look for more local profiles matching “'+q+'”.',
    mode:'directory_handoff',
    sources:[],
    links:[{label:language==='es'?'Ver más coincidencias':'See more matches',url:(language==='es'?'/es/directorio/?q=':'/directory/?q=')+encodeURIComponent(q)}],
    directoryQuery:q
  };
}
function isDirectoryFollowup(question){
  return /\b(show me more|find someone|find another|more options?|any others?|another one|near me|more like that|mu[eé]strame m[aá]s|buscar a alguien|otra opci[oó]n|m[aá]s opciones|cerca de m[ií])\b/i.test(text(question));
}
function profileCardFollowup(question,profiles,language){
  const rows=Array.isArray(profiles)?profiles.filter(Boolean):[];if(!rows.length)return null;
  const q=text(question).toLowerCase();
  const wantsWebsite=/\b(which|what|who|ones?|these|those|them|have|has|website|websites|site|sites|online)\b/.test(q)&&/\b(website|websites|web site|websites|site|sites|online)\b/.test(q)||/\b(cu[aá]l|cu[aá]les|estos|estas|tienen|sitio web|sitios web|p[aá]gina web|p[aá]ginas web)\b/.test(q)&&/\b(web|sitio|p[aá]gina)\b/.test(q);
  const wantsPhone=/\b(which|what|who|ones?|these|those|them|call|phone|phones|number|numbers|telephone)\b/.test(q)&&/\b(call|phone|phones|number|numbers|telephone)\b/.test(q)||/\b(cu[aá]l|cu[aá]les|estos|estas|llamar|tel[eé]fono|tel[eé]fonos|n[uú]mero|n[uú]meros)\b/.test(q)&&/\b(llamar|tel[eé]fono|n[uú]mero)\b/.test(q);
  if(!wantsWebsite&&!wantsPhone)return null;
  const matches=rows.filter(p=>wantsWebsite&&wantsPhone?(p.websiteHref||p.phoneHref):wantsWebsite?p.websiteHref:p.phoneHref);
  const noun=wantsWebsite&&wantsPhone?(language==='es'?'sitio web o teléfono':'a website or phone number'):wantsWebsite?(language==='es'?'sitio web':'website'):(language==='es'?'teléfono':'phone number');
  const answer=matches.length
    ?(language==='es'?`De los perfiles que acabo de mostrar, ${matches.length} ${matches.length===1?'incluye':'incluyen'} ${noun} en los datos públicos actuales de Franklin Navigator.`:`Of the profiles I just showed, ${matches.length} ${matches.length===1?'has':'have'} ${noun} in Franklin Navigator’s current public directory data.`)
    :(language==='es'?`Ninguno de los perfiles que acabo de mostrar incluye ${noun} en los datos públicos actuales de Franklin Navigator.`:`None of the profiles I just showed includes ${noun} in Franklin Navigator’s current public directory data.`);
  return {answer,mode:'directory_card_followup',sources:[],links:[],profiles:matches};
}
...
function install(root){
  ...
  const language=lang(),history=[];let busy=false,attachment=null,recognition=null,statusTurn=null,lastDirectoryQuery='',lastProfiles=[],chatHeader=null,profileContext=publicProfileContext(),profileContextBox=null;
  const formLabel=form.querySelector('label'),initialLabelText=text(formLabel?.textContent||'');
  output.classList.add('franklin-assistant-thread');
  const renderProfileContext=()=>{if(profileContextBox){profileContextBox.remove();profileContextBox=null}if(!profileContext)return;profileContextBox=make('aside','franklin-assistant-profile-context');profileContextBox.dataset.profileContext='public';const head=make('div','franklin-assistant-profile-context-head');head.append(make('strong','',language==='es'?'Ayuda con: '+profileContext.profileName:'Helping with: '+profileContext.profileName));const clearContext=make('button','link-button',language==='es'?'Quitar contexto del perfil':'Clear profile context');clearContext.type='button';clearContext.addEventListener('click',()=>{profileContext=null;clearPublicProfileContextFromUrl();renderProfileContext();input.focus()});head.append(clearContext);profileContextBox.append(head,make('p','fine-print',language==='es'?'Solo se usa la identidad pública del perfil y este paso del flujo. No se comparte estado privado de cuenta, pago ni evidencia.':'Only the public profile identity and this workflow step are used. No private account, payment, or evidence state is shared.'));const examples=make('div','actions');for(const [label,q] of (language==='es'?[["¿Cómo administro este perfil?","¿Cómo administro este perfil?"],["¿Puedo corregirlo gratis?","¿Puedo corregirlo gratis?"],["¿Qué pasa después de solicitar acceso?","¿Qué pasa después de solicitar acceso?"]]:[["How do I manage this profile?","How do I manage this profile?"],["Can I correct this for free?","Can I correct this for free?"],["What happens after I request access?","What happens after I request access?"]])){const b=make('button','button small',label);b.type='button';b.addEventListener('click',()=>{input.value=q;form.requestSubmit()});examples.append(b)}profileContextBox.append(examples);root.insertBefore(profileContextBox,form)};
  ...
  const activateConversationLayout=()=>{
    if(root.dataset.franklinConversationLayout==='1')return;
    root.dataset.franklinConversationLayout='1';
    chatHeader=make('div','franklin-assistant-chat-header');
    chatHeader.append(
      make('div','eyebrow','Franklin Assistant'),
      make('strong','',language==='es'?'Su conversación':'Your conversation')
    );
    root.insertBefore(chatHeader,output);
    output.insertAdjacentElement('afterend',form);
    form.classList.add('franklin-assistant-followup-composer');
    if(formLabel)formLabel.textContent=language==='es'?'Haga una pregunta de seguimiento':'Ask a follow-up';
    input.placeholder=language==='es'?'Haga una pregunta de seguimiento…':'Ask a follow-up…';
  };
  const restoreStartingLayout=()=>{
    delete root.dataset.franklinConversationLayout;
    form.classList.remove('franklin-assistant-followup-composer');
    if(chatHeader){chatHeader.remove();chatHeader=null}
    root.insertBefore(form,output);
    if(formLabel)formLabel.textContent=initialLabelText||(language==='es'?'Pregunte a Franklin':'Ask Franklin anything');
  };
  ...
  const finishTurn=(question,data)=>{if(statusTurn){statusTurn.remove();statusTurn=null}const turn=make('section','franklin-assistant-turn');turn.append(answerCard(data));output.append(turn);history.push({role:'user',content:question},{role:'assistant',content:text(data.answer)});if(history.length>12)history.splice(0,history.length-12);if(String(data.mode||'')==='directory_handoff')lastDirectoryQuery=data.directoryQuery||directoryQueryFromData(data)||lastDirectoryQuery;const shown=Array.isArray(data.profiles)?data.profiles.filter(Boolean):[],mode=String(data.mode||'');if(mode==='directory_handoff'&&shown.length)lastProfiles=shown.map(p=>({id:p.id||'',name:p.name||'',category:p.category||'',location:p.location||'',profile:p.profile||'',phone:p.phone||'',phoneHref:p.phoneHref||'',websiteHref:p.websiteHref||''}));else if(mode!=='directory_card_followup'&&mode!=='error')lastProfiles=[];input.value='';input.placeholder=language==='es'?'Haga una pregunta de seguimiento…':'Ask a follow-up…';input.focus();turn.scrollIntoView({block:'nearest',behavior:'smooth'})};
  const clearAll=()=>{history.splice(0);lastDirectoryQuery='';lastProfiles=[];attachment=null;attachInput.value='';attachStatus.textContent='';removeFile.hidden=true;input.value='';input.placeholder=language==='es'?'Pregunte sobre Franklin…':'Ask about Franklin…';output.replaceChildren();output.hidden=true;restoreStartingLayout();if(recognition){try{recognition.stop()}catch{}recognition=null}if(voiceStatus)voiceStatus.textContent='';input.focus()};
  ...
  window.FranklinAssistant=Object.freeze({version:VERSION,ask,clear:clearAll,history:()=>history.map(row=>({...row})),publicContext:()=>profileContext?{...profileContext}:null,clearProfileContext:()=>{profileContext=null;clearPublicProfileContextFromUrl();renderProfileContext()}});
}
