...
function install(root){
  ...
  const language=lang(),history=[];let busy=false,attachment=null,recognition=null,statusTurn=null,lastDirectoryQuery='';
  ...
  const clearAll=()=>{history.splice(0);lastDirectoryQuery='';attachment=null;attachInput.value='';attachStatus.textContent='';removeFile.hidden=true;input.value='';input.placeholder=language==='es'?'Pregunte sobre Franklin…':'Ask about Franklin…';output.replaceChildren();output.hidden=true;if(recognition){try{recognition.stop()}catch{}recognition=null}if(voiceStatus)voiceStatus.textContent='';input.focus()};
