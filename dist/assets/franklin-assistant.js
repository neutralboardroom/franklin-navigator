...
const VERSION='FRANKLIN-ASSISTANT2-0.3.5';
...
function install(root){
  ...
  const language=lang(),history=[];let busy=false,attachment=null,recognition=null,statusTurn=null,lastDirectoryQuery='',lastProfiles=[],chatHeader=null;
  ...
  const positionAtQuestion=turn=>{if(!turn)return;turn.style.scrollMarginTop='96px';try{input.focus({preventScroll:true})}catch{input.focus()}requestAnimationFrame(()=>turn.scrollIntoView({block:'start',behavior:'smooth'}))};
  const finishTurn=(question,data,userTurn)=>{if(statusTurn){statusTurn.remove();statusTurn=null}const turn=make('section','franklin-assistant-turn');turn.append(answerCard(data));output.append(turn);history.push({role:'user',content:question},{role:'assistant',content:text(data.answer)});if(history.length>12)history.splice(0,history.length-12);if(String(data.mode||'')==='directory_handoff')lastDirectoryQuery=data.directoryQuery||directoryQueryFromData(data)||lastDirectoryQuery;const shown=Array.isArray(data.profiles)?data.profiles.filter(Boolean):[],mode=String(data.mode||'');if(mode==='directory_handoff'&&shown.length)lastProfiles=shown.map(p=>({id:p.id||'',name:p.name||'',category:p.category||'',location:p.location||'',profile:p.profile||'',phone:p.phone||'',phoneHref:p.phoneHref||'',websiteHref:p.websiteHref||''}));else if(mode!=='directory_card_followup'&&mode!=='error')lastProfiles=[];input.value='';input.placeholder=language==='es'?'Haga una pregunta de seguimiento…':'Ask a follow-up…';positionAtQuestion(userTurn)};
  ...
  const ask=async raw=>{const question=text(raw).slice(0,600);if(busy||!question)return;activateConversationLayout();setBusy(true);const userTurn=addUser(question);userTurn.style.scrollMarginTop='96px';addStatus(language==='es'?'Buscando una respuesta clara…':'Finding a clear answer…');try{let data;if(attachment){data={answer:documentAnswer(attachment.text,question,language),mode:'attachment_on_device',sources:[],links:[]}}else{data=profileCardFollowup(question,lastProfiles,language)||(lastDirectoryQuery&&isDirectoryFollowup(question)?continuedDirectoryRequest(lastDirectoryQuery,language):ordinaryLawnService(question,language));if(!data){const response=await fetch(API,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({question,language,history:history.slice(-6)})});data=await response.json().catch(()=>({}));if(!response.ok||data?.ok!==true||!data?.answer)throw new Error(data?.error||'ANSWER_UNAVAILABLE')}}data=await enrichDirectoryHandoff(data,language);finishTurn(question,data,userTurn)}catch{if(statusTurn){statusTurn.remove();statusTurn=null}const turn=make('section','franklin-assistant-turn');turn.append(answerCard({answer:language==='es'?'No pude completar esa respuesta en este momento. Inténtelo de nuevo en un momento.':'I could not complete that answer right now. Please try again in a moment.',mode:'error',sources:[],links:[]}));output.append(turn);input.value=question;positionAtQuestion(userTurn)}finally{setBusy(false)}};
