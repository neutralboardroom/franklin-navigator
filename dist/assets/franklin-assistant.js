...
const VERSION='FRANKLIN-ASSISTANT2-0.3.4';
...
function install(root){
  ...
  const language=lang(),history=[];let busy=false,attachment=null,recognition=null,statusTurn=null,lastDirectoryQuery='',chatHeader=null;
  ...
  const finishTurn=(question,data)=>{if(statusTurn){statusTurn.remove();statusTurn=null}const turn=make('section','franklin-assistant-turn');turn.append(answerCard(data));output.append(turn);history.push({role:'user',content:question},{role:'assistant',content:text(data.answer)});if(history.length>12)history.splice(0,history.length-12);if(String(data.mode||'')==='directory_handoff')lastDirectoryQuery=data.directoryQuery||directoryQueryFromData(data)||lastDirectoryQuery;input.value='';input.placeholder=language==='es'?'Haga una pregunta de seguimiento…':'Ask a follow-up…';input.focus();turn.scrollIntoView({block:'nearest',behavior:'smooth'})};
  const clearAll=()=>{history.splice(0);lastDirectoryQuery='';attachment=null;attachInput.value='';attachStatus.textContent='';removeFile.hidden=true;input.value='';input.placeholder=language==='es'?'Pregunte sobre Franklin…':'Ask about Franklin…';output.replaceChildren();output.hidden=true;restoreStartingLayout();if(recognition){try{recognition.stop()}catch{}recognition=null}if(voiceStatus)voiceStatus.textContent='';input.focus()};
