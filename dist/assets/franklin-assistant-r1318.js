(()=>{'use strict';
const C=window.FranklinAssistantCore,R=window.FranklinAssistantR1296Core||window.FranklinAssistantR1295Core||window.FranklinAssistantR1294Core,F=window.FranklinAssistantFilesR1293,X=window.FranklinAssistantContextR1317,root=document.querySelector('[data-navigator-bot]');if(!C||!R||!X||!root)return;
...
const VERSION='FR-NAV1.30.18-HF3.13.0',ROUTE_RELEASE='FR-NAV1.29.2-HF3.10.2',COMMUNITY='FRANKLIN_TN',DIR_MANIFEST='/data/discovery/manifest.json',DIR_SHA='d97640231d541f5a4f67ac26782806fc933237a39566e6e59561ea82e894e225',ANSWER_API='https://franklin-navigator-assistant.onrender.com/api/answer';
let routePromise,dirPromise;const pageCache=new Map();root.dataset.franklinAssistantR1296='1';root.dataset.franklinAssistantVersion=VERSION;
...
function shouldRenderGuide(raw,effective,meta={}){
  if(meta.hasFile||Number(meta.profileHits||0)>0)return true;
  const t=C.norm(raw||effective||'');
  if(!t)return false;
  if(serviceActionFollow(raw))return true;
  return /\b(what should i do next|what do i do next|next step|next steps|help me find|find local|show local|show me local|list local|plan|planning|options|not sure|apply|application|permit steps|enrollment steps|hire|contact|call|phone number|website|websites|que debo hacer|qué debo hacer|siguiente paso|siguientes pasos|ayudame a buscar|ayúdame a buscar|buscar local|mostrar local|opciones|no estoy seguro|solicitar|solicitud|contratar|contactar|llamar|numero de telefono|número de teléfono|sitio web)\b/i.test(t);
}
...
async function answerFromRuntime(raw,effective=raw){
  ...
  try{
    ...
    output.dataset.answerMode=mode;
    output.dataset.llmUsed=d.llmUsed===true?'true':'false';
    const healthCode=mode==='clarify'?'assistant_answer_clarify':mode.includes('research')?'assistant_answer_research':mode.includes('known')?'assistant_answer_known':'assistant_answer_other';
    window.FranklinProductHealth?.record?.(healthCode);
    if(mode==='clarify'&&lastRuntimeAnswerMode==='clarify'){
      window.FranklinProductHealth?.record?.('assistant_repeated_clarification');
      window.FranklinIssueMonitorR1308?.report?.('ASSISTANT_REPEATED_CLARIFICATION',{workflow:'ASSISTANT',action:'conversation-quality',clientSignal:'CONSECUTIVE_CLARIFY_WITHOUT_RAW_CHAT'});
    }
    lastRuntimeAnswerMode=mode;
    ...
    window.FranklinProductHealth?.record?.('assistant_answer_runtime_failure');
    return false
  }
}
...
function inferService(raw){return X.inferService(raw,C)}
function serviceActionFollow(raw){return X.serviceActionFollow(raw,C)}
const conversation=[],chatHistory=[];const guideProgress={key:'',depth:0};let lastEffective='',lastService=null,lastTopics=[],lastProfileQuery='',lastRuntimeAnswerMode='',busy=false,returnFocus=null;
function contextualize(raw){return X.contextualize(raw,{lastEffective,lastService,lastTopics},C)}
...
function fallbackKnown(q){
  ...
  if(/\b(deck|decks|terraza|terrazas)\b/.test(t)&&/\b(repair|repairs|repairing|fix|permit|permits|reparar|reparacion|reparación|permiso|permisos)\b/.test(t))return es?'Sí. Para reparar una terraza en Franklin, cuente con necesitar un permiso de construcción. La Ciudad indica que se requiere permiso para terrazas y para la mayoría de los trabajos de reparación. Si es una reparación menor exactamente igual a lo existente, confirme el alcance con Building & Neighborhood Services al 615-794-7012 antes de comenzar.':'Yes. For deck repair in Franklin, plan on needing a building permit. The City says building permits are required for decks and for most repair work. If this is a very minor in-kind repair, confirm the exact scope with Building & Neighborhood Services at 615-794-7012 before starting.';
...
async function renderAnswer(raw,target,effectiveOverride=''){
  ...
  if(!answered&&profileIntent){
    ...
    if(dh.length&&dh[0].score>=18){answered=renderProfileAnswer(dh,displayQ||profileQuery)||answered;lastProfileQuery=profileQuery;if(answered)window.FranklinProductHealth?.record?.('assistant_provider_handoff')}
  }
  ...
  if(answered&&shouldRenderGuide(displayQ||q,q,{profileHits:dh.length,hasFile}))renderGuide(displayQ||q,q,{profileHits:dh.length,hasFile});
...
function makePanel(){
  ...
  d.querySelector('[data-chat-new]').addEventListener('click',()=>{conversation.length=0;chatHistory.length=0;lastEffective='';lastService=null;lastTopics=[];lastProfileQuery='';lastRuntimeAnswerMode='';guideProgress.key='';guideProgress.depth=0;transcript.replaceChildren();chatInput.value='';chatInput.focus();window.FranklinProductHealth?.record?.('assistant_r1307_new_chat')});
...
const publicApi=Object.freeze({version:VERSION,contextVersion:X.VERSION,send,renderInto:renderAnswer,conversation:()=>conversation.map(x=>({...x})),contextualizeForDiagnostics:(q,state)=>X.contextualize(q,state||{},C),shouldRenderGuideForDiagnostics:(q,state={})=>shouldRenderGuide(q,q,state)});window.FranklinAssistantR1318=publicApi;window.FranklinAssistantR1317=publicApi;window.FranklinAssistantR1316=publicApi;window.FranklinAssistantR1315=publicApi;window.FranklinAssistantR1307=publicApi;window.FranklinAssistantR1306=publicApi;window.FranklinAssistantR1305=publicApi;window.FranklinAssistantR1304=publicApi;window.FranklinAssistantR1303=publicApi;window.FranklinAssistantR1302=publicApi;window.FranklinAssistantR1301=publicApi;window.FranklinAssistantR1300=publicApi;window.FranklinAssistantR1299=publicApi;window.FranklinAssistantR1298=publicApi;window.FranklinAssistantR1296=publicApi;window.FranklinAssistantR1294=publicApi;
})();
