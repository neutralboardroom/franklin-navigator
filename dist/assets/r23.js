(() => {
  ...
  if(pricing){const sel=qs('[data-pricing-category]',pricing),plans=qs('[data-pricing-plans]',pricing),placeholder=qs('[data-pricing-placeholder]',pricing);let data=null;const money=n=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(n);fetch('/data/r23-pricing-preview.json').then(r=>r.json()).then(d=>data=d).catch(()=>{});sel?.addEventListener('change',()=>{const v=data?.verticals?.find(x=>x.id===sel.value);if(!v){plans.hidden=true;placeholder.hidden=false;return}placeholder.hidden=true;plans.hidden=false;plans.innerHTML=v.plans.map(p=>{const foundingMonthly=p.monthlyUsd*.7,foundingAnnual=p.annualUsd*.7;return `<article class="r22-card"><div class="eyebrow">${html(p.tier==='OFFICE'?'Office / Location':p.tier[0]+p.tier.slice(1).toLowerCase())}</div><h3>${html(p.tier==='INDIVIDUAL'?'One owner / professional':p.tier==='TEAM'?'Small team':'Office or location')}</h3><p><strong>${money(p.monthlyUsd)}/month</strong> or <strong>${money(p.annualUsd)}/year</strong> (12 months for the price of 10).</p><p class="fine-print"><strong>FOUNDING30:</strong> ${money(foundingMonthly)}/month equivalent or ${money(foundingAnnual)}/year for the first 12 months when enrolled by Dec. 31, 2026; then regular V2 price unless cancelled.</p><p class="fine-print">Checkout is not active yet.</p></article>`}).join('')+`<article class="r22-card"><div class="eyebrow">Enterprise</div><h3>Larger / multi-location organization</h3><p>Quote-only. Current category starting point: ${money(v.enterprise.startingMonthlyUsd)}/month or ${money(v.enterprise.startingAnnualUsd)}/year before custom scope.</p><p class="fine-print">Enterprise is not self-service checkout.</p></article>`})}
  // R34 profile-to-preparation loop: profile facts remain unchanged; this only adds a voluntary workflow link.
  const profileActions=qs('.profile-utility-actions');
  if(profileActions&&!qs('.r34-profile-prep',profileActions)){const a=document.createElement('a');a.className='r34-profile-prep';a.href='/preparation-studio/?source=profile';a.textContent='Prepare before contacting';profileActions.append(a)}
