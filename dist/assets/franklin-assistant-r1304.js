...
const VERSION='FR-NAV1.30.4-HF3.11.4',ROUTE_RELEASE='FR-NAV1.29.2-HF3.10.2',COMMUNITY='FRANKLIN_TN',DIR_MANIFEST='/data/discovery/manifest.json',DIR_SHA='d97640231d541f5a4f67ac26782806fc933237a39566e6e59561ea82e894e225',ANSWER_API='https://franklin-navigator-assistant.onrender.com/api/answer';
...
function wantsProfilesNow(raw){const t=C.norm(raw),svc=!!C.serviceFor(raw),explicit=/\b(find|show|list|name|names|best|top|recommend|recommended|looking for|hire|contact|call|phone|email|website|websites|near me|nearby|who can|which ones|buscar|mostrar|lista|nombre|nombres|mejor|mejores|recomendar|recomendado|contratar|contactar|llamar|telefono|correo|sitio web|cerca)\b/.test(t),needService=/\bneed (a|an|some)\b/.test(t)&&svc,permitOnly=/\b(permit|inspection|zoning|license|licensing)\b/.test(t)&&!explicit;return explicit||(needService&&!permitOnly)}
...
function requestedProfileCount(raw){const m=C.norm(raw).match(/\b([1-5])\b/);return m?Math.max(1,Math.min(5,Number(m[1]))):3}
function renderProfileAnswer(hits,q){
  if(!hits.length)return false;
  const count=Math.min(requestedProfileCount(q),hits.length),selected=hits.slice(0,count),t=C.norm(q);
  ...
  if(asksBest)intro=tx(
    `I can’t objectively verify which roofers are “best,” but here are ${count} matching Franklin-area roofing profiles from our public directory: ${names}. These are relevance matches, not endorsements—compare scope, insurance or licensing where applicable, price and availability directly.`,
    `No puedo verificar objetivamente cuáles techadores son “los mejores”, pero aquí tiene ${count} perfiles coincidentes del área de Franklin de nuestro directorio público: ${names}. Son coincidencias por relevancia, no recomendaciones; compare alcance, seguro o licencia cuando corresponda, precio y disponibilidad directamente.`
  );
...
async function renderAnswer(raw,target,effectiveOverride=''){
  ...
  const profileFollow=affirmativeProfileFollow(displayQ)&&!!C.serviceFor(q);
  const profileIntent=q&&(wantsProfilesNow(displayQ||q)||profileFollow)&&(R.shouldSearchProfiles(q)||!!C.serviceFor(q));
  ...
  window.FranklinProductHealth?.record?.(hasFile?'assistant_r1304_file':dh.length?'assistant_r1304_profiles':'assistant_r1304_answer');
}
function makePanel(){
