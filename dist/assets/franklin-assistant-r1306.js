...
const VERSION='FR-NAV1.30.6-HF3.11.6',ROUTE_RELEASE='FR-NAV1.29.2-HF3.10.2',COMMUNITY='FRANKLIN_TN',DIR_MANIFEST='/data/discovery/manifest.json',DIR_SHA='d97640231d541f5a4f67ac26782806fc933237a39566e6e59561ea82e894e225',ANSWER_API='https://franklin-navigator-assistant.onrender.com/api/answer';
...
function renderGuide(raw,effective,meta={}){
  const g=guideFor(raw,effective,meta);if(!g?.question)return false;
...
const conversation=[],chatHistory=[];let lastEffective='',lastService=null,lastTopics=[],lastProfileQuery='',busy=false,returnFocus=null;
...
async function renderAnswer(raw,target,effectiveOverride=''){
  ...
  const profileIntent=q&&(explicitProfile||profileFollow);
  const profileQuery=profileFollow?[lastProfileQuery,displayQ].filter(Boolean).join(' '):q;
  ...
  window.FranklinProductHealth?.record?.(hasFile?'assistant_r1306_file':dh.length?'assistant_r1306_profiles':'assistant_r1306_answer');
}
function makePanel(){
  ...
  d.querySelector('[data-chat-new]').addEventListener('click',()=>{conversation.length=0;chatHistory.length=0;lastEffective='';lastService=null;lastTopics=[];lastProfileQuery='';transcript.replaceChildren();chatInput.value='';chatInput.focus();window.FranklinProductHealth?.record?.('assistant_r1306_new_chat')});
