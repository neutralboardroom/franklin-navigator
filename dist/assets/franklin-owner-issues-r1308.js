...
const humanCode=v=>String(v||'Incident').toLowerCase().split('_').filter(Boolean).map(s=>s.charAt(0).toUpperCase()+s.slice(1)).join(' ');
...
function renderIncident(x){
  ...
  return `<article class="owner-incident" data-incident="${esc(x.incident_id)}">
    <h3>${esc(humanCode(x.safe_error_code))}</h3><p class="fine-print">${esc(x.workflow)} · ${esc(x.safe_error_code)}</p>
