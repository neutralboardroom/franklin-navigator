...
async function renderAnswer(raw,target,effectiveOverride=''){
  ...
  if(q&&(R.researchNeeded(q,confidence)||!answered)){
...
async function send(raw,panel=makePanel()){
  ...
  try{await renderAnswer(q,body,effective);recordTurn(q,effective)}catch(err){body.replaceChildren(el('p',tx('I could not finish that check. I can still help—try the question again or use the Franklin links below.','No pude terminar esa consulta. Aún puedo ayudarle: vuelva a intentarlo o use los enlaces de Franklin que aparecen abajo.')));body.append(actions([[tx('Open Help Center','Abrir Centro de ayuda'),'/community-help-center/'],[tx('Search Find Local','Buscar Find Local'),'/directory/']]))}finally{
