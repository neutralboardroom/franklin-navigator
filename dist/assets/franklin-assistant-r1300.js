...
const VERSION='FR-NAV1.30.0-HF3.11.0',ROUTE_RELEASE='FR-NAV1.29.2-HF3.10.2',COMMUNITY='FRANKLIN_TN',DIR_MANIFEST='/data/discovery/manifest.json',DIR_SHA='d97640231d541f5a4f67ac26782806fc933237a39566e6e59561ea82e894e225',RESEARCH='https://franklin-navigator-assistant.onrender.com/api/research';
...
const conversation=[];let lastEffective='',lastService=null,lastTopics=[],busy=false,returnFocus=null;
...
function recordTurn(raw,effective){conversation.push({role:'user',text:String(raw||'').trim()},{role:'assistant',context:effective});if(conversation.length>24)conversation.splice(0,conversation.length-24);lastEffective=effective;lastService=C.serviceFor(effective)||null;lastTopics=C.concepts(effective)}
async function renderAnswer(raw,target,effectiveOverride=''){
  ...
  const wait=el('p',tx('Checking Franklin Navigator…','Consultando Franklin Navigator…'),'franklin-chat-thinking');wait.setAttribute('role','status');output.append(wait);
  let rr=[],rh=[],dh=[];
  try{rr=await routes();if(q)rh=await routeHits(rr,q)}catch{}
  const profileIntent=q&&wantsProfilesNow(displayQ||q)&&R.shouldSearchProfiles(q);
  if(profileIntent){try{dh=C.rankDirectory(await directory(),q,5)}catch{}}
  ...
  const direct=q?R.direct(q,lang()):null,local=q&&rh.length&&(!direct||R.answerQuality(direct.text,q)<70)?await localAnswer(rh,q):null;
  output.replaceChildren();

  let answered=false;
  if(hasFile){answered=renderFile(displayQ||q)||answered}
  if(profileIntent&&dh.length){answered=renderProfileAnswer(dh,displayQ||q)||answered}
  if(!answered&&direct){renderDirect(direct);answered=true}
  if(!answered&&local){output.append(el('p',local.text));answered=true}

  const confidence=direct?.confidence||local||dh.length||rh[0]?.score>=35?'medium':'low';
  const freshAsk=/\b(open now|hours|today|tonight|tomorrow|current|currently|latest|available|availability|schedule|price|fee|cost|closed|closure|deadline|meeting|agenda|registration|register|abierto ahora|horario|hoy|esta noche|mañana|actual|último|disponible|precio|costo|cerrado|cierre|fecha límite|reunión|agenda|inscripción)\b/i.test(displayQ);
  const currentNeeded=q&&R.researchNeeded(q,confidence)&&(!profileIntent||freshAsk);
  if(currentNeeded){
    const fallback=[...output.childNodes].map(n=>n.cloneNode(true));
    output.replaceChildren();
    const researched=await webResearch(q);
    if(researched)answered=true;
    else if(fallback.length){output.replaceChildren(...fallback);answered=true}
  }else if(q&&!answered){
    const researched=await webResearch(q);
    answered=researched||answered;
  }
  if(!answered)output.append(el('p',tx('I do not have enough verified information for a confident answer yet. Tell me one more detail and I will answer directly.','Todavía no tengo suficiente información verificada para responder con confianza. Dígame un detalle más y responderé directamente.')));
  window.FranklinProductHealth?.record?.(hasFile?'assistant_r1300_file':dh.length?'assistant_r1300_profiles':rh.length?'assistant_r1300_local':'assistant_r1300_research');
}
function makePanel(){
  ...
  d.querySelector('[data-chat-new]').addEventListener('click',()=>{conversation.length=0;lastEffective='';lastService=null;lastTopics=[];transcript.replaceChildren();chatInput.value='';chatInput.focus();window.FranklinProductHealth?.record?.('assistant_r1296_new_chat')});
...
async function send(raw,panel=makePanel()){
  ...
  body.append(el('p',tx('Checking Franklin Navigator…','Consultando Franklin Navigator…'),'franklin-chat-thinking'));requestAnimationFrame(()=>scrollReplyToReadingStart(panel,row,{smooth:false}));
  try{await renderAnswer(q,body,effective);recordTurn(q,effective)}catch(err){body.replaceChildren(el('p',tx('I could not finish that answer. Please try the question again.','No pude terminar esa respuesta. Intente la pregunta de nuevo.')))}finally{
