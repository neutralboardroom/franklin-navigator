...
const VERSION='FR-NAV1.30.5-HF3.11.5',ROUTE_RELEASE='FR-NAV1.29.2-HF3.10.2',COMMUNITY='FRANKLIN_TN',DIR_MANIFEST='/data/discovery/manifest.json',DIR_SHA='d97640231d541f5a4f67ac26782806fc933237a39566e6e59561ea82e894e225',ANSWER_API='https://franklin-navigator-assistant.onrender.com/api/answer';
...
async function renderAnswer(raw,target,effectiveOverride=''){
  ...
  window.FranklinProductHealth?.record?.(hasFile?'assistant_r1305_file':dh.length?'assistant_r1305_profiles':'assistant_r1305_answer');
}
function makePanel(){
  ...
  d.querySelector('[data-chat-new]').addEventListener('click',()=>{conversation.length=0;chatHistory.length=0;lastEffective='';lastService=null;lastTopics=[];lastProfileQuery='';transcript.replaceChildren();chatInput.value='';chatInput.focus();window.FranklinProductHealth?.record?.('assistant_r1296_new_chat')});
