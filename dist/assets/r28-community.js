/* Franklin Navigator R28 compatibility helpers — R1327 current-price cleanup */
(() => {
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>[...r.querySelectorAll(s)];
  const money=n=>new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(n);
  const CURRENT_PLAN=Object.freeze({label:'Franklin Navigator Community Membership',price:35,billing:'per year',renewal:'renews annually until canceled',autoRenew:true,termMonths:12,lookupKey:'franklin_community_member_annual_v6'});
  const selectedPlan=()=>CURRENT_PLAN;
  ...
  for(const form of $$('[data-r28-plan-form]')){
    ...
    const render=()=>{const text=buildSummary(form);if(output)output.textContent=text;if(email)email.href='mailto:community@franklinnavigator.com?subject='+encodeURIComponent('See member options for my Franklin member profile')+'&body='+encodeURIComponent(text+'\n\nPlease show me the next step. No payment is authorized by this email.')};
  ...
  if(location.pathname==='/local-growth-engine/'&&!document.querySelector('[data-r28-growth-context]')){
    const main=$('main'),root=$('[data-local-growth-engine]');if(main&&root){const section=document.createElement('section');section.className='r28-community-context';section.dataset.r28GrowthContext='true';section.innerHTML='<div class="wrap"><div><strong>Community Membership supports local visibility and participation; it does not guarantee sales.</strong><div>Use this free planner to get started. Active members can also use guided Growth Desk planning, richer profile tools, and English, Spanish, or bilingual planning tools.</div></div><a class="button primary" href="/member-profile-preview/">See member options</a></div>';main.insertBefore(section,root)}
  }
})();
