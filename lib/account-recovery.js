...
const VERSION='FRANKLIN_ACCOUNT_RECOVERY_R1355';
const {PASSWORD_MIN_LENGTH}=require('./security');
...
function createAccountRecovery(deps={}){
  ...
  async function requestReset(req,res,reqId){
    ...
    if(emailRe.test(email)&&rateLimit(`password-reset-target:${targetKey(email)}`,3,60*60*1000)){
      ...
      if(account){
        ...
        const profileId=typeof body.profileId==='string'?body.profileId.trim():'';const returnMode=body.returnMode==='reviewer'?'reviewer':'';
        ...
        try{await sendResetEmail(account.email,raw,profileId,returnMode);}catch(error){
  ...
  async function completeReset(req,res,reqId){
    ...
    if(password.length<PASSWORD_MIN_LENGTH||password.length>256)throw publicError('PASSWORD_INVALID',`Use a password with at least ${PASSWORD_MIN_LENGTH} characters.`,400);
    ...
    await tx(async client=>{
      ...
      await client.query(`delete from franklin_review_sessions where account_id=$1`,[accountId]).catch(()=>{});
      await audit(client,'ACCOUNT',accountId,'PASSWORD_RESET_COMPLETED','ACCOUNT',accountId,reqId,{sessionsInvalidated:true,reviewSessionsInvalidated:true});
    });
...
module.exports={createAccountRecovery,VERSION,PURPOSE,RESET_MINUTES,GENERIC_MESSAGE,PASSWORD_MIN_LENGTH};
