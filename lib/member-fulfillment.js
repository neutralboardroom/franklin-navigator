 ...
 return {version:VERSION,reviewCoverageConfigured:reviewers.size>0,publication,async route(req,res,url,reqId){
   ...
   if(url.pathname.startsWith('/admin/member-review/')){
     ...
     if(req.method==='GET'&&url.pathname.endsWith('/queue')){
       ...
       const rows=(await query(`select r.*,a.email_normalized as requester_email from ${table} r join franklin_accounts a on a.account_id=r.account_id where r.state=$1 and a.community='FRANKLIN_TN' and a.state='ACTIVE' order by r.updated_at,r.account_id,r.profile_id limit 51 offset $2`,[state,offset])).rows;
