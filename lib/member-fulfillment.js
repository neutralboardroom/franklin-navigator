...
const VERSION='FRANKLIN_MEMBER_FULFILLMENT_HF2';
...
function createMemberWorkflow(d){
 ...
 return {version:VERSION,reviewCoverageConfigured:reviewers.size>0,publication,async route(req,res,url,reqId){
   ...
   if(req.method==='GET'&&url.pathname==='/api/member/public-profile'){const p=profile(url.searchParams.get('profileId'));return send(200,{publication:await publication({query},p)});}
