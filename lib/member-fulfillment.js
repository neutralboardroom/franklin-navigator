...
// R1338: explicit claim consent, status-aware recovery, and fail-safe self-release without changing Profile Factory facts.
const VERSION='FRANKLIN_MEMBER_FULFILLMENT_HF3_4';
 ...
 return {version:VERSION,reviewCoverageConfigured:reviewers.size>0,publication,async route(req,res,url,reqId){
   ...
   const result=await tx(async c=>{await lock(c,p);
     ...
     if(url.pathname==='/api/member/representation/release'){
       ...
       const rev=(row?.revision||0)+1;
       await c.query("update franklin_profile_links set authority_state='REVOKED',verified_at=null,updated_at=now() where account_id=$1 and profile_id=$2",[a,p]);
