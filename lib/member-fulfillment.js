...
function createMemberWorkflow(d){
 ...
 return {version:VERSION,reviewCoverageConfigured:reviewers.size>0,publication,async route(req,res,url,reqId){
   ...
   if(req.method==='GET'&&url.pathname==='/api/member/public-profile'){const p=profile(url.searchParams.get('profileId'));const managed=Boolean((await query("select 1 from franklin_profile_links l join franklin_accounts a using(account_id) where l.profile_id=$1 and l.authority_state='VERIFIED' and a.state='ACTIVE' limit 1",[p])).rowCount);return send(200,{managedProfile:managed,activePaidMember:await publicEntitlement({query},p),publication:await publication({query},p)});}
   if(req.method==='GET'&&url.pathname==='/api/member/sponsored-profiles'){
     ...
     const limit=Math.max(1,Math.min(3,Number(url.searchParams.get('limit')||2)||2));
     ...
     const candidates=rows.map(r=>String(r.profile_id||'')).filter(p=>p&&p!==exclude&&Object.hasOwn(scope.profiles,p))
       .sort((a,b)=>digest(day+'|'+exclude+'|'+a).localeCompare(digest(day+'|'+exclude+'|'+b)));
     const selected=[];
