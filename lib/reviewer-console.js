...
const {ownerBinding}=require('./owner-review-setup');
const VERSION = 'FRANKLIN_REVIEWER_CONSOLE_2';
...
function createReviewerConsole(d) {
  ...
  const api={version:VERSION,configured,origin,allowedReviewerIds:configured?[...reviewers]:[],
    ...
    prepareReviewBody(req,b){
      ...
      const firstPartyOwnerException=Boolean(who&&who.account_id===ownerAccountId&&b.accountId===who.account_id&&b.profileId==='FR-ORG-b00c0ace7943973c');
      if(who&&b.accountId===who.account_id&&!firstPartyOwnerException)fail('REVIEW_SELF_DECISION_FORBIDDEN',403);
    ...
    async route(req,res,url,reqId,workflow,readyError,mediaWorkflow){
      ...
      try {
        ...
        if(url.pathname==='/api/reviewer/login' && req.method==='POST') {
          ...
          if(!passwordOk || !binding || a.state!=='ACTIVE' || !a.email_verified_at)fail('REVIEW_SIGN_IN_FAILED',401);
