...
const {randomToken, sha256, safeEqual, verifyPassword, hashPassword, PASSWORD_MIN_LENGTH} = require('./security');
const {ownerBinding,confirmEmail}=require('./owner-review-setup');
const VERSION = 'FRANKLIN_REVIEWER_CONSOLE_4';
...
function createReviewerConsole(d) {
  ...
  async function authorize(req,client=d,locking=false) {
    ...
    const r=(await client.query(`select s.account_id,s.reviewer_id,s.binding_sha256,s.csrf_hash,s.expires_at,s.credential_sha256,a.password_hash,a.email_verified_at from franklin_review_sessions s join franklin_accounts a using(account_id) where s.session_hash=$1 and s.community='FRANKLIN_TN' and s.expires_at>now() and a.state='ACTIVE'${locking?' for share of s,a':''}`,[tokenHash(token)])).rows[0];
    const b=r&&bindings.get(r.account_id);
    if(!r || !r.email_verified_at || !b || b.reviewerId!==r.reviewer_id || !safeEqual(b.digest,r.binding_sha256) || !safeEqual(sha256(csrf),r.csrf_hash) || !safeEqual(tokenHash(r.password_hash),r.credential_sha256))fail('REVIEW_SIGN_IN_REQUIRED',401);
  ...
  const api={version:VERSION,configured,origin,allowedReviewerIds:configured?[...reviewers]:[],
    reviewerAccessForAccount:accountId=>Boolean(configured&&bindings.has(String(accountId||''))),
    protectedAdminAccessForAccount:(accountId,profileId)=>Boolean(configured&&ownerAccountId&&String(accountId||'')===ownerAccountId&&String(profileId||'')==='FR-ORG-b00c0ace7943973c'),
    ...
    prepareReviewBody(req,b){
      ...
      const who=contexts.get(req);
      // No ordinary reviewer may decide their own profile-access request. Even the
      // platform-owner account must use an independent reviewer or a separately
      // authorized owner-override path that is not this ordinary review form.
      if(who&&b.accountId===who.account_id)fail('REVIEW_SELF_DECISION_FORBIDDEN',403);
    ...
    sendMemberJson(req,res,status,payload,reqId){
      ...
      if(payload.items)payload={ok:true,kind:payload.kind,nextOffset:payload.nextOffset,pendingCount:Number(payload.pendingCount||0),items:payload.items.map(r=>({accountId:r.account_id,requesterEmail:r.requester_email||'',profileId:r.profile_id,profileName:d.profileNames[r.profile_id]?.name || 'Franklin profile',revision:r.revision,state:r.state,statement:r.statement||'',evidenceUrl:r.evidence_url||'',fields:r.fields||null,createdAt:r.created_at||null,updatedAt:r.updated_at||null,authorityState:r.authority_state||null,existingManagerCount:Number(r.existing_manager_count||0),submissionCount:Number(r.submission_count||0),selfReviewConflict:Boolean(contexts.get(req)&&r.account_id===contexts.get(req).account_id),rightsConfirmed:Boolean(r.rights_confirmed_at)}))};
      send(res,status,payload);
    },
    async route(req,res,url,reqId,workflow,readyError,mediaWorkflow){
      const assets={'/review/':['index.html','text/html'],'/review/app.js':['app.js','text/javascript'],'/review/style.css':['style.css','text/css'],'/review/reset/':['reset.html','text/html'],'/review/reset.js':['reset.js','text/javascript']};
      ...
      try {
        ...
        if(url.pathname==='/api/reviewer/login' && req.method==='POST') {
          ...
          if(password.length<PASSWORD_MIN_LENGTH || password.length>256 || email.length>254)fail('REVIEW_SIGN_IN_FAILED',401);
          ...
          await d.tx(async c=>{
            ...
            const current=(await c.query("select password_hash,email_verified_at from franklin_accounts where account_id=$1 and state='ACTIVE' for share",[a.account_id])).rows[0];
            if(!current || current.password_hash!==a.password_hash || !current.email_verified_at)fail('REVIEW_SIGN_IN_FAILED',401);
          ...
          return send(res,200,{ok:true,csrf,expiresAt,reviewerEmail:email,reviewerId:binding.reviewerId});
        }
        ...
        if(url.pathname.startsWith('/api/reviewer/media/')) {
          if(!mediaWorkflow)fail('REVIEW_ACCESS_UNAVAILABLE',503);
          const map={'/api/reviewer/media/queue':'/admin/member-media/queue','/api/reviewer/media/file':'/admin/member-media/file','/api/reviewer/media/history':'/admin/member-media/history','/api/reviewer/media/review':'/admin/member-media/review'};
          const targetPath=map[url.pathname];if(!targetPath)fail('NOT_FOUND',404);
          if(targetPath.endsWith('/review')&&req.method!=='POST')fail('METHOD_NOT_ALLOWED',405);
          if(!targetPath.endsWith('/review')&&req.method!=='GET')fail('METHOD_NOT_ALLOWED',405);
          const target=new URL(url.href);target.pathname=targetPath;
          await mediaWorkflow.route(req,res,target,reqId,{reviewer:who.reviewer_id,reviewerAccount:who.account_id,allowSelfReviewProfileId:who.account_id===ownerAccountId?'FR-ORG-b00c0ace7943973c':''});return true;
        }
        ...
        if(url.pathname==='/api/reviewer/submissions' && req.method==='GET') {
          const p=url.searchParams.get('profileId'),a=url.searchParams.get('accountId');
          if(!Object.hasOwn(d.profileNames,p||'') || !/^acct_[A-Za-z0-9]{8,80}$/.test(a||''))fail('REVIEW_ITEM_INVALID',400);
          const rows=(await d.query(`select revision,evidence_url,statement,submitted_at from franklin_representation_submissions where community='FRANKLIN_TN' and profile_id=$1 and account_id=$2 order by revision desc limit 20`,[p,a])).rows;
          return send(res,200,{ok:true,submissions:rows});
        }
