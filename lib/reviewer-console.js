...
function createReviewerConsole(d) {
  ...
  try { bindings=parseBindings(env.REVIEWER_ACCOUNT_BINDINGS,reviewers); origin=reviewerOrigin(env); configured=bindings.size>0 && String(env.SESSION_SECRET||'').length>=32; }
  ...
  const api={version:VERSION,configured,origin,
