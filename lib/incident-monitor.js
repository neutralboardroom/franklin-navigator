...
function notificationDecision(row,last,{now=Date.now(),criticalCooldownMinutes=15,highCooldownMinutes=60,updateMinOccurrences=5,durationEscalationMinutes=120}={}){
  ...
  const lastSeenMs=Date.parse(row.last_seen||'')||0;
  const activitySinceLastNotification=delta>0||(lastSeenMs>0&&lastSeenMs>lastAt);
  if(!activitySinceLastNotification)return null;
  const firstMs=Date.parse(row.first_seen||'')||now;
  const durationMinutes=Math.max(0,(now-firstMs)/60000);
  if(delta>=updateMinOccurrences||(durationMinutes>=durationEscalationMinutes&&lastSeenMs>lastAt))return'UPDATE';
  return null;
}
function legacyNotificationFallback(row){
  if(!row?.last_alert_at)return null;
  const status=String(row.status||'OPEN').toUpperCase();
  return {
    kind:status==='RESOLVED'?'RESOLVED':'INITIAL',
    severity:String(row.severity||'NORMAL'),
    count:Number(row.occurrence_count||0),
    state:status,
    at:row.last_alert_at,
    legacy:true
  };
}
...
function createIncidentMonitor({query,tx,newId,sha256,release='UNKNOWN',environment='PRODUCTION'}){
  ...
  async function lastNotification(incidentId,fallbackRow=null){
    ...
    return legacyNotificationFallback(fallbackRow);
  }
  ...
  async function deliverPendingAlerts(){
    ...
    deliveryPromise=(async()=>{
      if(!externalDeliveryConfigured())return {configured:false,delivery:'CONFIGURATION_AUTHORITY_REQUIRED',evaluated:0,attempted:0,networkAttempted:0,delivered:0,failed:0,suppressed:0};
      const rows=(await query(`
        ...
        limit 100
      `)).rows;
      let delivered=0,failed=0,suppressed=0,networkAttempted=0;
      for(const row of rows){
        const last=await lastNotification(row.incident_id,row);
      ...
      return {configured:true,delivery:'RESEND_EMAIL_CONFIGURED',evaluated:rows.length,attempted:networkAttempted,networkAttempted,delivered,failed,suppressed};
    })();
  ...
  async function setStatus(incidentId,status,note='',options={}){
    ...
    if(next==='RESOLVED'&&notify)scheduleAlertDelivery();
  ...
  async function scanDerivedIssues(){
    ...
    await safeScan('legacy_origin_preflight_storm_reconciliation',async()=>{
      const rows=(await query(`
        select incident_id
        from franklin_incidents
        where status in ('OPEN','ACKNOWLEDGED')
          and safe_error_code='ORIGIN_NOT_ALLOWED'
          and safe_context->>'path'='/api/telemetry/issue'
          and coalesce(safe_context->>'method',safe_context->>'operation')='OPTIONS'
          and coalesce(safe_context->>'environment','')=''
      ...
      for(const row of rows)await setStatus(
        row.incident_id,
        'RESOLVED',
        'R1353 legacy alert-storm reconciliation: historical per-client telemetry preflight incident superseded by the stable route/method/cause fingerprint. Origin rejection remains enforced and future occurrences remain recorded.',
        {notify:false}
      );
      return rows.length;
    });
  ...
  async function selfTest(){
    ...
    const originA=await persistRecord({category:'PUBLIC_SITE',workflow:'PUBLIC_SITE',code:'ORIGIN_NOT_ALLOWED',source:'STARTUP_SELF_TEST',accountId:'synthetic-a',context:{path:'/api/telemetry/issue',method:'OPTIONS',httpStatus:403,originClass:'BLOCKED_PREFLIGHT',environment:'SELF_TEST'}});
    const originB=await persistRecord({category:'PUBLIC_SITE',workflow:'PUBLIC_SITE',code:'ORIGIN_NOT_ALLOWED',source:'STARTUP_SELF_TEST',accountId:'synthetic-b',context:{path:'/api/telemetry/issue',method:'OPTIONS',httpStatus:403,originClass:'BLOCKED_PREFLIGHT',environment:'SELF_TEST'}});
...
module.exports={VERSION,COMMUNITY,createIncidentMonitor,redactContext,severityFor,safeWorkflow,safeCode,fingerprintParts,normalizedCause,notificationDecision,legacyNotificationFallback};
