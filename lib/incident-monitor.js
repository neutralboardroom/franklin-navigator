...
function createIncidentMonitor({query,tx,newId,sha256,release='UNKNOWN',environment='PRODUCTION'}){
  ...
  async function lastNotification(incidentId,fallbackRow=null){
    ...
    if(fallbackRow?.last_alert_at){
      return {
        kind:'INITIAL',
        severity:String(fallbackRow.severity||'NORMAL'),
        count:Number(fallbackRow.occurrence_count||0),
        state:String(fallbackRow.status||'OPEN'),
        at:fallbackRow.last_alert_at,
        legacy:true
      };
    }
    return null;
