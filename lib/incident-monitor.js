...
const VERSION='FRANKLIN_ISSUE_MONITOR_2';
...
function createIncidentMonitor({query,tx,newId,sha256}){
  ...
  function alertText(row){
    ...
    return [
      'Franklin Navigator owner incident alert',
      '',
      'Community: '+COMMUNITY,
  ...
  async function sendResendAlert(row){
    ...
    const response=await fetch('https://api.resend.com/emails',{
      ...
      body:JSON.stringify({
        ...
        to:[alertConfig.to],
        subject:'[Franklin Navigator] '+clean(row.severity,16)+' incident: '+clean(row.workflow,60)+' / '+safeCode(row.safe_error_code),
        text:alertText(row)
      }),
  ...
  async function deliverPendingAlerts(){
    ...
    const rows=(await query(`
      select incident_id,category,severity,workflow,safe_error_code,status,safe_context,profile_id,
             occurrence_count,first_seen,last_seen,last_alert_at
      from franklin_incidents
      where status='OPEN' and severity in ('CRITICAL','HIGH')
        and (
          last_alert_at is null
          or (severity='CRITICAL' and last_alert_at<now()-interval '15 minutes')
          or (severity='HIGH' and last_alert_at<now()-interval '60 minutes')
        )
      order by case severity when 'CRITICAL' then 1 else 2 end,last_seen desc
      limit 20
    `)).rows;
    let delivered=0,failed=0;
    for(const row of rows){
      try{
        await sendResendAlert(row);
    ...
    return {configured:true,delivery:'RESEND_EMAIL_CONFIGURED',attempted:rows.length,delivered,failed};
  }
  ...
  async function persistRecord(input={},client=null){
    ...
    return {...row,category,workflow,code,externalDelivery:externalDeliveryStatus()};
  }
  ...
  async function summary(){
    ...
    return {community:COMMUNITY,counts,openCritical,openHigh,externalDelivery:externalDeliveryStatus(),ownerVisibility:true,version:VERSION};
  }
  ...
  async function selfTest(){
    ...
    return {ok:true,dedup:true,redaction:true,resolution:true,externalDelivery:externalDeliveryStatus(),alertDeliveryConfigured:externalDeliveryConfigured(),pending:pending.length};
  }

  return Object.freeze({version:VERSION,record,flushPending,list,summary,setStatus,scanDerivedIssues,deliverPendingAlerts,readinessSnapshot,selfTest,redactContext,severityFor,externalDeliveryConfigured,externalDeliveryStatus,pendingCount:()=>pending.length});
}
