...
function createIncidentMonitor({query,tx,newId,sha256}){
  ...
  async function record(input={},client=null){
    try{return await persistRecord(input,client)}
      ...
      return {queued:true,severity:severityFor({workflow:queued.workflow,code:queued.code,httpStatus:queued.httpStatus,hint:queued.severity,count:1}),externalDelivery:'CONFIGURATION_AUTHORITY_REQUIRED'};
    }
  }
  ...
  async function selfTest(){
    ...
    return {ok:true,dedup:true,redaction:true,resolution:true,externalDelivery:'CONFIGURATION_AUTHORITY_REQUIRED',pending:pending.length};
  }

  return Object.freeze({version:VERSION,record,flushPending,list,summary,setStatus,scanDerivedIssues,readinessSnapshot,selfTest,redactContext,severityFor,pendingCount:()=>pending.length});
}
