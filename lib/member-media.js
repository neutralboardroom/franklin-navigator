...
function validateWebp(buf,maxBytes){
 ...
 // Canvas-generated WebP should not carry EXIF/XMP metadata. Reject it at the trust boundary.
 for(let i=12;i+8<=buf.length;){const chunk=buf.toString('ascii',i,i+4),len=buf.readUInt32LE(i+4);if(chunk==='EXIF'||chunk==='XMP ')fail('MEDIA_METADATA_NOT_ALLOWED',415);i+=8+len+(len%2);if(i>buf.length+1)fail('MEDIA_TYPE_INVALID',415);}
 return {mimeType:'image/webp',byteSize:buf.length,sha256:sha256(buf)};
}
...
function createMemberMedia(d){
 ...
 return {version:VERSION,
  async route(req,res,url,reqId,context={}){
   ...
   const result=await tx(async c=>{const row=(await c.query("select * from franklin_member_media where media_id=$1 and account_id=$2 and profile_id=$3 for update",[mediaId,a,p])).rows[0];if(!row)fail('MEDIA_NOT_FOUND',404);
     if(url.pathname==='/api/member/media/submit'){await slotAllowed(c,a,p,row.slot);if(!['DRAFT','CHANGES_REQUESTED'].includes(row.state))fail('MEDIA_DRAFT_REQUIRED',409);if(b.rightsConfirmed!==true)fail('PUBLICATION_PERMISSION_REQUIRED');await c.query("update franklin_member_media set state='SUBMITTED',rights_confirmed_at=now(),public_reason='',updated_at=now() where media_id=$1",[mediaId]);await audit(c,'ACCOUNT',a,'MEMBER_MEDIA_SUBMITTED','PROFILE',p,reqId,{mediaId,slot:row.slot,position:row.position});return {mediaId,state:'SUBMITTED'};}
     if(url.pathname==='/api/member/media/remove'){await authority(c,a,p);await c.query("update franklin_member_media set state='REMOVED',removed_at=now(),updated_at=now() where media_id=$1",[mediaId]);await audit(c,'ACCOUNT',a,'MEMBER_MEDIA_REMOVED','PROFILE',p,reqId,{mediaId,slot:row.slot,position:row.position});return {mediaId,state:'REMOVED'};}
     fail('NOT_FOUND',404);
   });return send(200,result);
  }
 };
