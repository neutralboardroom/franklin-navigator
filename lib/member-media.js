  ...
  async route(req,res,url,reqId,context={}){
   ...
   if(adminPaths.has(url.pathname)){
    ...
    if(req.method==='GET'&&url.pathname==='/admin/member-media/queue'){
      ...
      return send(200,{kind:'media',items:rows.slice(0,50).map(r=>({...r,profileName:profiles[r.profile_id]?.name||'Franklin profile',previewUrl:'/api/reviewer/media/file?mediaId='+encodeURIComponent(r.media_id),rightsConfirmed:Boolean(r.rights_confirmed_at)})),nextOffset:rows.length>50?offset+50:null});
    }
