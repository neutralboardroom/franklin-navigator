...
function mapStripeEvent(event) {
  ...
  const linePeriodEnd = object.lines?.data?.reduce((latest,line)=>Math.max(latest,Number(line?.period?.end||0)),0) || 0;
  const itemPeriodEnd = object.items?.data?.reduce((latest,item)=>Math.max(latest,Number(item?.current_period_end||0)),0) || 0;
  const providerPeriodEnd = Math.max(linePeriodEnd,itemPeriodEnd);
  let currentPeriodEnd = object.current_period_end ? new Date(object.current_period_end * 1000).toISOString() : providerPeriodEnd ? new Date(providerPeriodEnd * 1000).toISOString() : null;
  ...
  if (!currentPeriodEnd && ['checkout.session.completed','checkout.session.async_payment_succeeded'].includes(type) && object.mode === 'payment' && termMonths === 36 && autoRenew === 'false') {
  ...
  if (type === 'customer.subscription.updated') {
    if (object.cancel_at_period_end) return {type:'CANCEL_AT_PERIOD_END',created,subscriptionId:object.id,currentPeriodEnd};
    if (['past_due','unpaid','incomplete','incomplete_expired','paused'].includes(String(object.status))) return {type:'PAYMENT_FAILED',created,subscriptionId:object.id,currentPeriodEnd};
    return {type:'CANCEL_REVERSED',created,subscriptionId:object.id,currentPeriodEnd};
  }
  if (type === 'customer.subscription.deleted') return {type:'TERMINATED', created, subscriptionId:object.id, currentPeriodEnd};
  if (type === 'charge.refunded' && Number(object.amount_refunded||0) >= Number(object.amount||0) && Number(object.amount||0) > 0) return {type:'REFUNDED', created, subscriptionId, currentPeriodEnd};
  return null;
}
