...
const {ownerBinding,confirmEmail,resetPasswordWithEmailProof}=require('./owner-review-setup');
  ...
  const api={version:VERSION,configured,origin,allowedReviewerIds:configured?[...reviewers]:[],
    ...
    async route(req,res,url,reqId,workflow,readyError,mediaWorkflow){
      ...
      try {
        ...
        if(!configured)fail('REVIEW_ACCESS_UNAVAILABLE',503);
        if(url.pathname==='/api/reviewer/password-reset' && req.method==='POST') {
          const b=await d.readBody(req),email=String(b.email||'').trim().toLowerCase(),code=String(b.emailCode||''),newPassword=String(b.newPassword||'');
          if(!d.rateLimit('review-reset-ip:'+sha256(req.socket.remoteAddress||''),12,900000) || !d.rateLimit('review-reset-account:'+sha256(email),4,900000))fail('REVIEW_RATE_LIMITED',429);
          if(email.length>254)fail('REVIEW_PASSWORD_RESET_FAILED',401);
          ...
          await resetPasswordWithEmailProof(d,env,a,binding,email,code,newPassword,reqId);
        ...
        if(url.pathname.startsWith('/api/reviewer/media/')) {
          ...
          await mediaWorkflow.route(req,res,target,reqId,{reviewer:who.reviewer_id,reviewerAccount:who.account_id});return true;
        }
