...
const RELEASE = process.env.LOCAL_RELEASE || 'FR-NAV1.10.0-CANDIDATE-R35';
...
async function route(req,res){const reqId=requestId();try{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);if(req.method==='OPTIONS'){const origin=String(req.headers.origin||'');if(origin&&!ALLOWED_ORIGINS.has(origin))throw publicError('ORIGIN_NOT_ALLOWED','This request origin is not allowed.',403);return sendEmpty(req,res,204);}enforceOrigin(req);
  ...
  if(req.method==='POST'&&url.pathname==='/api/membership/start'){if(!COMMERCE_ENABLED)throw publicError('COMMERCE_DISABLED','Franklin Navigator membership checkout is not open yet.',423);if(!checkoutInfrastructureConfigured())throw publicError('CHECKOUT_INFRASTRUCTURE_NOT_READY','Secure membership checkout is not ready yet.',503);const session=await requireSession(req);const body=await readBody(req);const profileId=normalizeProfile(body.profileId);const plan=getPlan(body.lookupKey);if(!plan)throw publicError('PLAN_INVALID','Choose an available Franklin membership plan.');const verified=await query(`select 1 from franklin_profile_links where account_id=$1 and profile_id=$2 and authority_state='VERIFIED'`,[session.account_id,profileId]);if(!verified.rowCount)throw publicError('PROFILE_VERIFICATION_REQUIRED','Verify your connection to this Franklin profile before enrollment.',409);const existingResult=await query(`select * from franklin_memberships where account_id=$1 and profile_id=$2 order by updated_at desc limit 1`,[session.account_id,profileId]);const existing=existingResult.rows[0]||null;if(existing&&accessAllowed(existing.status,existing.current_period_end))throw publicError('MEMBERSHIP_ALREADY_ACTIVE','This Franklin profile already has an active membership.',409);const intentId=newId('checkout');await query(`insert into franklin_checkout_intents(intent_id,account_id,profile_id,lookup_key,expires_at) values($1,$2,$3,$4,now()+interval '24 hours')`,[intentId,session.account_id,profileId,plan.lookupKey]);try{const checkout=await createStripeCheckoutSession({plan,intentId,email:session.email,profileId,stripeCustomerId:existing?.stripe_customer_id});await query(`update franklin_checkout_intents set stripe_checkout_session_id=$2,state='SESSION_CREATED',updated_at=now() where intent_id=$1`,[intentId,checkout.id]);return sendJson(req,res,200,{ok:true,checkoutUrl:checkout.url,intentId,checkoutSessionId:checkout.id,plan:{lookupKey:plan.lookupKey,priceUsd:plan.regularUsd,autoRenew:plan.autoRenew,termMonths:plan.termMonths,billingMode:plan.billingMode,stripePriceId:plan.stripePriceId}},reqId);}catch(error){await query(`update franklin_checkout_intents set state='SESSION_CREATE_FAILED',updated_at=now() where intent_id=$1`,[intentId]).catch(()=>{});throw error;}}
