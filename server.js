...
const RELEASE = process.env.LOCAL_RELEASE || 'FR-NAV1.15.0-HF2.1-CANDIDATE';
...
const profileNames=JSON.parse(fs.readFileSync(path.join(__dirname,'data/member-profile-scope.json'),'utf8')).profiles;
const reviewerConsole=createReviewerConsole({query,tx,readBody,sendJson,audit,rateLimit,profileNames});
const memberWorkflow=createMemberWorkflow({query,tx,requireSession,readBody,sendJson:reviewerConsole.sendMemberJson,normalizeProfile,newId,audit,accessAllowed,safeEqual,adminToken:ADMIN_TOKEN,rateLimit,trustedReviewer:reviewerConsole.trustedReviewer,prepareReviewBody:reviewerConsole.prepareReviewBody,persistEvidence:reviewerConsole.persistEvidence,assertTrustedReviewer:reviewerConsole.assertTrustedReviewer});
async function route(req,res){const reqId=requestId();try{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);if(await reviewerConsole.route(req,res,url,reqId,memberWorkflow,readyError))return;if(req.method==='OPTIONS'){const origin=String(req.headers.origin||'');if(origin&&!ALLOWED_ORIGINS.has(origin))throw publicError('ORIGIN_NOT_ALLOWED','This request origin is not allowed.',403);return sendEmpty(req,res,204);}enforceOrigin(req);
