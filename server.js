...
const {createMemberRecognition,VERSION:MEMBER_RECOGNITION_VERSION}=require('./lib/member-recognition');
const {createProfileInvitations,VERSION:PROFILE_INVITATIONS_VERSION}=require('./lib/profile-invitations');
...
const CLAIM_WORKFLOW_VERSION='FRANKLIN_CLAIM_WORKFLOW_R1358_1';
const MANAGER_PROFILE_VERSION='FRANKLIN_VERIFIED_MANAGER_DIRECT_R1359_1';
const PROFILE_ALIAS_VERSION='FRANKLIN_PROFILE_ALIAS_R1360_1';
...
let readyError = null;
let profileAliases=Object.freeze({});
let profileAliasReconciliation={state:'NOT_RUN',aliasCount:0,rowsMoved:0,conflicts:0};
...
function claimDecisionCopy(decision){
  return {
    ...
    CHANGES_REQUESTED:{subject:'More information is needed for your Franklin Navigator profile request',heading:'More information needed',body:'Franklin Navigator needs more information before this profile-access request can be approved. Sign in to review the request and submit updated evidence.'},
    REJECTED:{subject:'Update on your Franklin Navigator profile-access request',heading:'Profile access was not approved',body:'Franklin Navigator did not approve this profile-access request. Sign in to review the decision message and next steps.'},
    REVOKED:{subject:'Your Franklin Navigator profile access changed',heading:'Profile access changed',body:'Management access for this Franklin Navigator profile is no longer active. Sign in to review the current status or contact support if you believe this is incorrect.'}
  }[decision]||null;
}
...
function issueWorkflow(pathname){
  ...
  if(p.includes('/representation')||p.includes('/profile-links')||p.includes('/claim')||p.includes('/profile-invitations'))return'PROFILE_CLAIM';
...
function normalizeProfile(value){const id=String(value||'').trim();if(!PROFILE_RE.test(id))throw publicError('PROFILE_ID_INVALID','Choose a valid Franklin Navigator profile.');return profileAliases[id]||id;}
...
async function processCanonicalEvent(client,eventKey,source,event,payloadHash,reqId){const mapped=mapStripeEvent(event);if(!mapped)return {state:'IGNORED'};const context=await resolveEventContext(client,event);let membership=context.membership;let accountId=membership?.account_id||context.intent?.account_id;let profileId=membership?.profile_id||context.intent?.profile_id;let lookupKey=membership?.lookup_key||context.intent?.lookup_key;if(!accountId||!profileId||!getPlan(lookupKey)){await client.query(`insert into franklin_dead_letters(dead_letter_id,source_event_key,reason_code,safe_context) values($1,$2,'MEMBERSHIP_CONTEXT_NOT_FOUND',$3::jsonb)`,[newId('dead'),eventKey,JSON.stringify({eventType:event.type})]);await incidentMonitor.record({category:'PAYMENT_MEMBERSHIP',workflow:'WEBHOOK',code:'DEAD_LETTER_OPEN',severity:'HIGH',source:'STRIPE_EVENT',requestId:reqId,context:{eventType:event.type,reasonCode:'MEMBERSHIP_CONTEXT_NOT_FOUND'}},client);return{state:'DEAD_LETTER'};}
  ...
  await memberRecognition.syncMembership(client,membership,{eventKey});
...
async function initializeMemberRecognition(){const sql=fs.readFileSync(path.join(__dirname,'schema','008_member_recognition.sql'),'utf8');const digest=sha256(sql);await tx(async c=>{await c.query('select pg_advisory_xact_lock(3316407,0)');const prior=(await c.query('select digest_sha256 from franklin_schema_migrations where version=$1',[MEMBER_RECOGNITION_VERSION])).rows[0];if(prior&&prior.digest_sha256!==digest)throw new Error('member_recognition_migration_digest_mismatch');if(!prior){await c.query(sql);await c.query('insert into franklin_schema_migrations(version,digest_sha256) values($1,$2)',[MEMBER_RECOGNITION_VERSION,digest]);}});}
async function initializeProfileInvitations(){const sql=fs.readFileSync(path.join(__dirname,'schema','009_profile_invitations.sql'),'utf8');const digest=sha256(sql);await tx(async c=>{await c.query('select pg_advisory_xact_lock(3316408,0)');const prior=(await c.query('select digest_sha256 from franklin_schema_migrations where version=$1',[PROFILE_INVITATIONS_VERSION])).rows[0];if(prior&&prior.digest_sha256!==digest)throw new Error('profile_invitations_migration_digest_mismatch');if(!prior){await c.query(sql);await c.query('insert into franklin_schema_migrations(version,digest_sha256) values($1,$2)',[PROFILE_INVITATIONS_VERSION,digest]);}});}
async function initializeClaimWorkflow(){const sql=fs.readFileSync(path.join(__dirname,'schema','010_claim_workflow.sql'),'utf8');const digest=sha256(sql);await tx(async c=>{await c.query('select pg_advisory_xact_lock(3316409,0)');const prior=(await c.query('select digest_sha256 from franklin_schema_migrations where version=$1',[CLAIM_WORKFLOW_VERSION])).rows[0];if(prior&&prior.digest_sha256!==digest)throw new Error('claim_workflow_migration_digest_mismatch');if(!prior){await c.query(sql);await c.query('insert into franklin_schema_migrations(version,digest_sha256) values($1,$2)',[CLAIM_WORKFLOW_VERSION,digest]);}});}
async function initializeVerifiedManagerProfile(){const sql=fs.readFileSync(path.join(__dirname,'schema','011_verified_manager_profile.sql'),'utf8');const digest=sha256(sql);await tx(async c=>{await c.query('select pg_advisory_xact_lock(3316410,0)');const prior=(await c.query('select digest_sha256 from franklin_schema_migrations where version=$1',[MANAGER_PROFILE_VERSION])).rows[0];if(prior&&prior.digest_sha256!==digest)throw new Error('manager_profile_migration_digest_mismatch');if(!prior){await c.query(sql);await c.query('insert into franklin_schema_migrations(version,digest_sha256) values($1,$2)',[MANAGER_PROFILE_VERSION,digest]);}});}
async function initializeProfileAliasRegistry(){
  const sql=fs.readFileSync(path.join(__dirname,'schema','012_profile_alias_reconciliation.sql'),'utf8');const digest=sha256(sql);
  await tx(async c=>{await c.query('select pg_advisory_xact_lock(3316411,0)');const prior=(await c.query('select digest_sha256 from franklin_schema_migrations where version=$1',[PROFILE_ALIAS_VERSION])).rows[0];if(prior&&prior.digest_sha256!==digest)throw new Error('profile_alias_migration_digest_mismatch');if(!prior){await c.query(sql);await c.query('insert into franklin_schema_migrations(version,digest_sha256) values($1,$2)',[PROFILE_ALIAS_VERSION,digest]);}});
}
async function reconcileProfileAliases(){
  const entries=Object.entries(profileAliases);if(!entries.length){profileAliasReconciliation={state:'PASS',aliasCount:0,rowsMoved:0,conflicts:0};return profileAliasReconciliation;}
  return tx(async c=>{
    await c.query('select pg_advisory_xact_lock(3316411,1)');
    for(const [aliasId,canonicalId] of entries)await c.query(`insert into franklin_profile_aliases(alias_profile_id,canonical_profile_id,source_release,source_artifact_sha256) values($1,$2,'FR-PF-PLATFORM-15.37','3feac2b4f8c912e2dced0ef9bcea76902b91776651a0829d9914693d78fff4b7') on conflict(alias_profile_id) do update set canonical_profile_id=excluded.canonical_profile_id,source_release=excluded.source_release,source_artifact_sha256=excluded.source_artifact_sha256,updated_at=now()`,[aliasId,canonicalId]);
    const checks=[
      ['franklin_profile_links','account_id'],['franklin_memberships','account_id'],['franklin_representation_reviews','account_id'],['franklin_member_drafts','account_id'],['franklin_reviews','account_id']
    ];
    let conflicts=0;const detail=[];
    for(const [table,key] of checks){const q=await c.query(`select count(*)::int count from ${table} a join franklin_profile_aliases m on m.alias_profile_id=a.profile_id join ${table} z on z.${key}=a.${key} and z.profile_id=m.canonical_profile_id`);const n=Number(q.rows[0]?.count||0);conflicts+=n;if(n)detail.push({table,count:n});}
    for(const [table,extra] of [['franklin_member_publications','true'],['franklin_member_recognition_years','z.recognition_year=a.recognition_year'],['franklin_member_decal_fulfillment','z.recognition_year=a.recognition_year']]){const q=await c.query(`select count(*)::int count from ${table} a join franklin_profile_aliases m on m.alias_profile_id=a.profile_id join ${table} z on z.profile_id=m.canonical_profile_id and ${extra}`);const n=Number(q.rows[0]?.count||0);conflicts+=n;if(n)detail.push({table,count:n});}
    for(const [table,extra] of [['franklin_representation_submissions','z.account_id=a.account_id and z.revision=a.revision'],['franklin_manager_profile_history','z.account_id=a.account_id and z.revision=a.revision'],['franklin_purchase_reservations','z.context=a.context and z.account_id=a.account_id']]){const q=await c.query(`select count(*)::int count from ${table} a join franklin_profile_aliases m on m.alias_profile_id=a.profile_id join ${table} z on z.profile_id=m.canonical_profile_id and ${extra}`);const n=Number(q.rows[0]?.count||0);conflicts+=n;if(n)detail.push({table,count:n});}
    if(conflicts){profileAliasReconciliation={state:'BLOCKED_CONFLICT',aliasCount:entries.length,rowsMoved:0,conflicts,detail};throw Object.assign(new Error('profile_alias_reconciliation_conflict'),{code:'PROFILE_ALIAS_RECONCILIATION_CONFLICT'});}
    const tables=['franklin_profile_links','franklin_checkout_intents','franklin_memberships','franklin_support_requests','franklin_onboarding','franklin_purchase_reservations','franklin_representation_reviews','franklin_member_drafts','franklin_member_publications','franklin_member_review_history','franklin_member_value_receipts','franklin_member_media','franklin_member_media_review_history','franklin_reviews','franklin_member_recognition_years','franklin_member_decal_fulfillment','franklin_profile_invitations','franklin_representation_submissions','franklin_claim_notification_log','franklin_manager_profile_history','franklin_incidents'];
    let rowsMoved=0;const moved={};
    for(const table of tables){const q=await c.query(`update ${table} t set profile_id=m.canonical_profile_id from franklin_profile_aliases m where t.profile_id=m.alias_profile_id`);const n=Number(q.rowCount||0);rowsMoved+=n;if(n)moved[table]=n;}
    const remaining=(await c.query(`select count(*)::int count from (select profile_id from franklin_profile_links union all select profile_id from franklin_memberships union all select profile_id from franklin_checkout_intents union all select profile_id from franklin_representation_reviews union all select profile_id from franklin_member_drafts union all select profile_id from franklin_member_publications union all select profile_id from franklin_reviews union all select profile_id from franklin_profile_invitations) x join franklin_profile_aliases m on m.alias_profile_id=x.profile_id`)).rows[0];
    if(Number(remaining?.count||0)!==0)throw new Error('profile_alias_reconciliation_incomplete');
    profileAliasReconciliation={state:'PASS',aliasCount:entries.length,rowsMoved,conflicts:0,moved};
    return profileAliasReconciliation;
  });
}

const runtimeProfileScope=loadProfileScope();
profileAliases=runtimeProfileScope.aliases||Object.freeze({});
...
const memberWorkflow=createMemberWorkflow({reviewers:reviewerConsole.allowedReviewerIds.length?reviewerConsole.allowedReviewerIds.join(','):process.env.MEMBER_REVIEWERS,query,tx,requireSession,readBody,sendJson:reviewerConsole.sendMemberJson,normalizeProfile,newId,audit,accessAllowed,safeEqual,adminToken:ADMIN_TOKEN,rateLimit,trustedReviewer:reviewerConsole.trustedReviewer,prepareReviewBody:reviewerConsole.prepareReviewBody,persistEvidence:reviewerConsole.persistEvidence,assertTrustedReviewer:reviewerConsole.assertTrustedReviewer,notifyRepresentationDecision:sendClaimDecisionNotification,protectedAdminAccessForAccount:reviewerConsole.protectedAdminAccessForAccount,scope:runtimeProfileScope});
...
const memberRecognition=createMemberRecognition({query,tx,requireSession,readBody,sendJson,normalizeProfile,newId,audit,accessAllowed,safeEqual,adminToken:ADMIN_TOKEN,scope:runtimeProfileScope,publicOrigin:PUBLIC_ORIGIN});
const profileInvitations=createProfileInvitations({query,tx,readBody,sendJson,requireSession,normalizeProfile,newId,audit,rateLimit,safeEqual,adminToken:ADMIN_TOKEN,sha256,randomToken,normalizeEmail,emailRe:EMAIL_RE,profileNames,publicOrigin:PUBLIC_ORIGIN});
async function route(req,res){const reqId=requestId();let routePath='/';try{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);routePath=url.pathname;if(await reviewerConsole.route(req,res,url,reqId,memberWorkflow,readyError,memberMedia))return;if(req.method==='OPTIONS'){const origin=String(req.headers.origin||'');if(origin&&!ALLOWED_ORIGINS.has(origin))throw publicError('ORIGIN_NOT_ALLOWED','This request origin is not allowed.',403);return sendEmpty(req,res,204);}enforceOrigin(req);
  ...
  if(await profileInvitations.route(req,res,url,reqId))return;if(await communityReviews.route(req,res,url,reqId))return;if(await memberMedia.route(req,res,url,reqId))return;if(await memberWorkflow.route(req,res,url,reqId))return;if(await memberRecognition.route(req,res,url,reqId))return;
async function reconcileProtectedAdminAccess(session,reqId){
  if(!session||!reviewerConsole.protectedAdminAccessForAccount?.(session.account_id,PROTECTED_PLATFORM_PROFILE_ID))return false;
  return tx(async client=>{
    await client.query('select pg_advisory_xact_lock(3316410,hashtext($1))',[PROTECTED_PLATFORM_PROFILE_ID]);
    const prior=(await client.query('select authority_state,verified_at from franklin_profile_links where account_id=$1 and profile_id=$2 for update',[session.account_id,PROTECTED_PLATFORM_PROFILE_ID])).rows[0]||null;
    const review=(await client.query('select state,revision from franklin_representation_reviews where account_id=$1 and profile_id=$2 for update',[session.account_id,PROTECTED_PLATFORM_PROFILE_ID])).rows[0]||null;
    const changed=!prior||prior.authority_state!=='VERIFIED'||Boolean(review&&review.state!=='VERIFIED');
    await client.query(`insert into franklin_profile_links(link_id,account_id,profile_id,authority_state,verified_at)
      values($1,$2,$3,'VERIFIED',now())
      on conflict(account_id,profile_id) do update set authority_state='VERIFIED',verified_at=coalesce(franklin_profile_links.verified_at,now()),updated_at=now()`,[newId('link'),session.account_id,PROTECTED_PLATFORM_PROFILE_ID]);
    if(review&&review.state!=='VERIFIED')await client.query(`update franklin_representation_reviews set state='VERIFIED',revision=revision+1,public_reason='Protected Franklin administrator access is active through the platform owner binding. Ordinary self-review was not used.',reviewed_by_hash=null,evidence_receipt=null,updated_at=now() where account_id=$1 and profile_id=$2`,[session.account_id,PROTECTED_PLATFORM_PROFILE_ID]);
    if(changed)await audit(client,'SYSTEM','PROTECTED_ADMIN_BINDING','PROTECTED_ADMIN_ACCESS_RECONCILED','PROFILE',PROTECTED_PLATFORM_PROFILE_ID,reqId,{accountId:session.account_id,priorAuthority:prior?.authority_state||null,priorReview:review?.state||null});
    return changed;
  });
}
  if(req.method==='GET'&&url.pathname==='/health'){let database=false;try{database=Boolean((await query('select 1 ok')).rowCount);}catch{}const cfg=configStatus();const healthy=cfg.ok&&database&&!readyError;return sendJson(req,res,healthy?200:503,{ok:healthy,release:RELEASE,checkoutSafetyVersion:CHECKOUT_SAFETY_VERSION,memberFulfillmentVersion:MEMBER_FULFILLMENT_VERSION,memberMediaVersion:MEMBER_MEDIA_VERSION,communityReviewsVersion:COMMUNITY_REVIEWS_VERSION,reviewerConsoleVersion:REVIEWER_CONSOLE_VERSION,memberRecognitionVersion:MEMBER_RECOGNITION_VERSION,issueMonitorVersion:ISSUE_MONITOR_VERSION,accountRecoveryVersion:ACCOUNT_RECOVERY_VERSION,profileInvitationsVersion:PROFILE_INVITATIONS_VERSION,claimWorkflowVersion:CLAIM_WORKFLOW_VERSION,managerProfileVersion:MANAGER_PROFILE_VERSION,profileAliasVersion:PROFILE_ALIAS_VERSION,profileAliasReconciliation,reviewerConsoleConfigured:reviewerConsole.configured,reviewCoverageConfigured:memberWorkflow.reviewCoverageConfigured,community:COMMUNITY,commerceEnabled:COMMERCE_ENABLED,databaseConfigured:Boolean(DATABASE_URL),database,startupReady:!readyError,missing:cfg.missing,ownerAlertDeliveryConfigured:incidentMonitor.externalDeliveryConfigured(),ownerAlertDelivery:incidentMonitor.externalDeliveryStatus(),uptimeSeconds:Math.floor(process.uptime())},reqId);}
  if(req.method==='GET'&&url.pathname==='/ready'){let database=false,migration=null;try{database=Boolean((await query('select 1 ok')).rowCount);migration=(await query(`select version,digest_sha256,applied_at from franklin_schema_migrations where version=$1`,[SCHEMA_VERSION])).rows[0]||null;}catch{}const cfg=configStatus();const infrastructureReady=cfg.ok&&database&&stripeKeyConfigured()&&stripeWebhookConfigured()&&Boolean(migration)&&!readyError;return sendJson(req,res,infrastructureReady?200:503,{ok:infrastructureReady,release:RELEASE,checkoutSafetyVersion:CHECKOUT_SAFETY_VERSION,memberFulfillmentVersion:MEMBER_FULFILLMENT_VERSION,reviewerConsoleVersion:REVIEWER_CONSOLE_VERSION,memberRecognitionVersion:MEMBER_RECOGNITION_VERSION,issueMonitorVersion:ISSUE_MONITOR_VERSION,accountRecoveryVersion:ACCOUNT_RECOVERY_VERSION,profileInvitationsVersion:PROFILE_INVITATIONS_VERSION,claimWorkflowVersion:CLAIM_WORKFLOW_VERSION,profileAliasVersion:PROFILE_ALIAS_VERSION,profileAliasReconciliation,reviewerConsoleConfigured:reviewerConsole.configured,reviewCoverageConfigured:memberWorkflow.reviewCoverageConfigured,community:COMMUNITY,schemaVersion:SCHEMA_VERSION,schemaDigest,migration,database,stripeCheckoutSessionConfigured:stripeKeyConfigured(),stripeWebhookConfigured:stripeWebhookConfigured(),portalSessionConfigured:stripeKeyConfigured(),commerceEnabled:COMMERCE_ENABLED,liveCheckoutEnabled:infrastructureReady&&COMMERCE_ENABLED,checkoutContract:'SERVER_CREATED_STRIPE_CHECKOUT_SESSION_V1',ownerAlertDeliveryConfigured:incidentMonitor.externalDeliveryConfigured(),ownerAlertDelivery:incidentMonitor.externalDeliveryStatus(),missing:cfg.missing,startupError:readyError?String(readyError.message||readyError):null},reqId);}
  ...
  if(req.method==='POST'&&url.pathname==='/api/telemetry/issue'){
    ...
    const result=await incidentMonitor.record({
      ...
      requestId:reqId,accountId:session?.account_id||clientKey(req),profileId:body.profileId?normalizeProfile(body.profileId):null,
  ...
  if(req.method==='GET'&&url.pathname==='/api/accounts/me'){const session=await requireSession(req);await reconcileProtectedAdminAccess(session,reqId);const links=await query(`select l.profile_id,l.authority_state,l.verified_at,r.state as review_state,r.revision as review_revision,r.created_at as review_created_at,r.updated_at as review_updated_at from franklin_profile_links l left join franklin_representation_reviews r using(account_id,profile_id) where l.account_id=$1 order by l.created_at`,[session.account_id]);const membership=await membershipForAccount(session.account_id);const protectedAdminProfileIds=reviewerConsole.protectedAdminAccessForAccount?.(session.account_id,PROTECTED_PLATFORM_PROFILE_ID)?[PROTECTED_PLATFORM_PROFILE_ID]:[];return sendJson(req,res,200,{ok:true,account:session,profileLinks:links.rows,membership,reviewerAccessAvailable:reviewerConsole.reviewerAccessForAccount(session.account_id),protectedAdminProfileIds},reqId);}
  if(req.method==='POST'&&url.pathname==='/api/profile-links'){const session=await requireSession(req);const body=await readBody(req);const profileId=normalizeProfile(body.profileId);if(!Object.hasOwn(profileNames,profileId))throw publicError('PROFILE_NOT_AVAILABLE','Choose a current Franklin Navigator profile.',404);if(profileId==='FR-ORG-b00c0ace7943973c'){const existing=await query('select authority_state from franklin_profile_links where account_id=$1 and profile_id=$2',[session.account_id,profileId]);if(existing.rowCount)return sendJson(req,res,200,{ok:true,profileId,authorityState:existing.rows[0].authority_state,reused:true},reqId);throw publicError('PROFILE_SYSTEM_MANAGED','This official Franklin Navigator profile is managed through protected administrator access. You can still submit a free factual correction.',403);}const linkId=newId('link');await query(`insert into franklin_profile_links(link_id,account_id,profile_id) values($1,$2,$3) on conflict(account_id,profile_id) do update set updated_at=now()`,[linkId,session.account_id,profileId]);return sendJson(req,res,201,{ok:true,profileId,authorityState:'PENDING'},reqId);}
...
async function start(){try{await initializeDatabase();await initializeProfileAliasRegistry();await reconcileProfileAliases();await reconcileR1312SyntheticSupportArtifacts();const monitorSelfTest=await incidentMonitor.selfTest();console.log(JSON.stringify({event:'FRANKLIN_INCIDENT_MONITOR_SELF_TEST',release:RELEASE,...monitorSelfTest,at:nowIso()}));await incidentMonitor.scanDerivedIssues();
...
module.exports={route,processCanonicalEvent,handleEvent,configStatus,normalizeProfile,memberLifecycleSummary,incidentMonitor,memberRecognition,commerceEnabled:COMMERCE_ENABLED};
