...
const {assertClaimableProfile}=require('./lib/profile-identity-safety');
...
async function reconcileProtectedAdminAccess(session,reqId){
  ...
  if(req.method==='POST'&&url.pathname==='/api/membership/start'){
    ...
    const session=await requireSession(req);const body=await readBody(req);const profileId=normalizeProfile(body.profileId);assertClaimableProfile(profileId,(code,status,message)=>{throw publicError(code,message,status)});const plan=getPlan(body.lookupKey);
