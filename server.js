...
const ISSUE_LIFECYCLE_MIGRATION='FRANKLIN_ISSUE_LIFECYCLE_1';

const RELEASE = process.env.LOCAL_RELEASE || 'FR-NAV1.30.53-HF3.13.35';
...
async function initializeIssueLifecycle(){const sql=fs.readFileSync(path.join(__dirname,'schema','008_incident_lifecycle.sql'),'utf8');const digest=sha256(sql);await tx(async c=>{await c.query('select pg_advisory_xact_lock(3316407,0)');const prior=(await c.query('select digest_sha256 from franklin_schema_migrations where version=$1',[ISSUE_LIFECYCLE_MIGRATION])).rows[0];if(prior&&prior.digest_sha256!==digest)throw new Error('issue_lifecycle_migration_digest_mismatch');if(!prior){await c.query(sql);await c.query('insert into franklin_schema_migrations(version,digest_sha256) values($1,$2)',[ISSUE_LIFECYCLE_MIGRATION,digest]);}});}
...
async function route(req,res){const reqId=requestId();let routePath='/';try{const url=new URL(req.url,`http://${req.headers.host||'localhost'}`);routePath=url.pathname;if(await reviewerConsole.route(req,res,url,reqId,memberWorkflow,readyError,memberMedia))return;if(req.method==='OPTIONS'){const origin=String(req.headers.origin||'');if(origin&&!ALLOWED_ORIGINS.has(origin))throw publicError('ORIGIN_NOT_ALLOWED','This request origin is not allowed.',403);return sendEmpty(req,res,204);}enforceOrigin(req);
  ...
  if(url.pathname.startsWith('/admin/')){const token=String(req.headers.authorization||'').replace(/^Bearer\s+/i,'');if(!ADMIN_TOKEN||!safeEqual(token,ADMIN_TOKEN))throw publicError('ADMIN_AUTH_REQUIRED','Administrative authorization required.',401);
    ...
    if(req.method==='GET'&&url.pathname==='/admin/incidents/digest')return sendJson(req,res,200,{ok:true,...await incidentMonitor.digestSnapshot(url.searchParams.get('limit')||50)},reqId);
...
setInterval(async()=>{try{await incidentMonitor.scanDerivedIssues();const delivery=await incidentMonitor.deliverPendingAlerts();const digest=await incidentMonitor.deliverDigestIfDue();if(delivery.attempted||delivery.failed)console.log(JSON.stringify({event:'FRANKLIN_OWNER_ALERT_DELIVERY',release:RELEASE,...delivery,at:nowIso()}));if(digest.attempted)console.log(JSON.stringify({event:'FRANKLIN_OWNER_ALERT_DIGEST',release:RELEASE,...digest,at:nowIso()}));}catch(error){console.error(JSON.stringify({event:'FRANKLIN_INCIDENT_SCAN_FAILED',code:error.code||error.constructor?.name,at:nowIso()}));}},300000).unref();}catch(error){readyError=error;console.error(JSON.stringify({event:'FRANKLIN_RUNTIME_STARTUP_NOT_READY',release:RELEASE,error:String(error.message||error),at:nowIso()}));}server.listen(PORT,'0.0.0.0',()=>console.log(JSON.stringify({event:'FRANKLIN_RUNTIME_LISTENING',release:RELEASE,port:PORT,ready:!readyError,issueMonitorVersion:ISSUE_MONITOR_VERSION,at:nowIso()})));}
