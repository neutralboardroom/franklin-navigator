...
add('R1352 selected claim panel keeps four required actions with clearer optional membership label',['Claim or manage this profile','Correct information','Request removal','See Community Membership benefits'].every(x=>claim.includes(x)));
...
add('R1352 authority submit has visible loading, duplicate suppression and prominent pending focus',live.includes('Submitting your request. Please keep this page open.')&&live.includes("pendingCard.dataset.r1346PendingStatus=''")&&live.includes("focusIntoView('[data-r1346-pending-status]')")&&live.includes('SUBMISSION_STATUS_UNCERTAIN'));
