...
add('Sep20 reset request has visible loading, duplicate suppression, generic success and check-email next step',recovery.includes("submit.textContent='Sending password-reset instructions…'")&&recovery.includes('submit.disabled=true')&&recovery.includes('If an account exists for this email, password-reset instructions have been sent.')&&recovery.includes('Check your email'));
add('Sep20 reset completion clears/replaces old form and visibly confirms signed-in exact-profile continuation',recovery.includes("a.input.value='';b.input.value=''")&&recovery.includes('body.replaceChildren()')&&recovery.includes('Password changed successfully.')&&recovery.includes('You’re signed in.')&&recovery.includes("profile?'/profile-access/'+profileQuery")&&recovery.includes('go.href=destination'));
